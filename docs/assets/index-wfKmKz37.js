(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();const fe=[];let se=null;function j(e,t){const s=[],a=new RegExp("^"+e.replace(/:([^/]+)/g,(n,r)=>(s.push(r),"([^/]+)"))+"$");fe.push({pattern:e,regex:a,keys:s,handler:t})}function je(e){se=e}function w(e){window.location.hash=e.startsWith("#")?e:`#${e}`}function oe(){const e=window.location.hash.slice(1)||"/";for(const t of fe){const s=e.match(t.regex);if(s){const a={};t.keys.forEach((n,r)=>{a[n]=decodeURIComponent(s[r+1])}),t.handler(a);return}}se&&se(e)}function Te(){window.addEventListener("hashchange",oe),window.location.hash?oe():window.location.hash="#/"}const ge="action-app:progress",be="action-app:recent-programs",Pe=5;function ae(){try{return JSON.parse(localStorage.getItem(ge)||"{}")}catch{return{}}}function ve(e){try{localStorage.setItem(ge,JSON.stringify(e))}catch{}}function he(e){const t=ae();return new Set(t[e]||[])}function Ie(e,t){const s=ae(),a=new Set(s[e]||[]);return a.has(t)?a.delete(t):a.add(t),s[e]=Array.from(a),ve(s),a}function Ae(e){const t=ae();delete t[e],ve(t)}function ye(){try{const e=JSON.parse(localStorage.getItem(be)||"[]");return Array.isArray(e)?e:[]}catch{return[]}}function Ne(e){if(e)try{const t=ye().filter(s=>s.id!==e);t.unshift({id:e,visitedAt:Date.now()}),localStorage.setItem(be,JSON.stringify(t.slice(0,Pe)))}catch{}}const S={workouts:null,exercises:null,plans:null,exerciseMap:null};async function z(){if(S.workouts)return S.workouts;const e=await fetch("./workouts.json");if(!e.ok)throw new Error(`Failed to load workouts.json: ${e.status}`);return S.workouts=await e.json(),S.workouts}async function A(){if(S.exercises)return S.exercises;const e=await fetch("./exercises.json");if(!e.ok)throw new Error(`Failed to load exercises.json: ${e.status}`);return S.exercises=await e.json(),S.exerciseMap=new Map(S.exercises.exercises.map(t=>[t.id,t])),S.exercises}async function we(){if(S.plans)return S.plans;const e=await fetch("./plans.json");if(!e.ok)throw new Error(`Failed to load plans.json: ${e.status}`);return S.plans=await e.json(),S.plans}async function Be(e){return(await z()).programs.find(s=>s.id===e)||null}async function Ue(e){return await A(),S.exerciseMap?.get(e)||null}async function He(e){const[t]=await Promise.all([Be(e),A()]);if(!t)return null;const s=r=>{const o=S.exerciseMap.get(r.exerciseId)||null;return{kind:"single",exerciseId:r.exerciseId,exercise:o,name:o?.name||r.exerciseId,reps:r.reps??o?.recommendations?.reps,sets:r.sets??o?.recommendations?.sets,repUnits:r.repUnits??o?.recommendations?.repUnits,note:r.note??o?.recommendations?.note,tags:r.tags||[]}},a=r=>({kind:r.kind,note:r.note,tags:r.tags||[],exercises:r.exercises.map(o=>{const i=S.exerciseMap.get(o.exerciseId)||null;return{exerciseId:o.exerciseId,exercise:i,name:i?.name||o.exerciseId,reps:o.reps??i?.recommendations?.reps,sets:o.sets??i?.recommendations?.sets,repUnits:o.repUnits??i?.recommendations?.repUnits,note:o.note??i?.recommendations?.note}})}),n=(t.items||[]).map(r=>r.kind?a(r):s(r));return{...t,resolvedItems:n}}function _e(){return["localhost","127.0.0.1"].includes(window.location.hostname)}function ze(e){e.innerHTML=`
      ...
      <div class="px-4 pb-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-slate-800/50 rounded-xl p-3 text-center">
            <p class="text-3xl font-extrabold text-brand-400 leading-none num tracking-tight">${B(e.reps||"—")}</p>
            <p class="label-meta mt-1.5">${B(e.repUnits||"reps")}</p>
          </div>
          <div class="bg-slate-800/50 rounded-xl p-3 text-center">
            <p class="text-3xl font-extrabold text-brand-400 leading-none num tracking-tight">${B(e.sets||"—")}</p>
            <p class="label-meta mt-1.5">sets</p>
          </div>
        </div>
        ${e.note?`
          <div class="bg-brand-500/10 border-l-2 border-brand-500 px-3 py-2.5 rounded-r-lg">
            <p class="text-sm text-slate-300 leading-relaxed">${B(e.note)}</p>
          </div>
        `:""}
      </div>
    </div>
  `,t.isExpanded&&a.length>0){const o=s.querySelector("[data-media-slot]");o&&_(o,a)}return s.querySelector('[data-action="toggle"]')?.addEventListener("click",()=>{t.onToggle?.(t.index)}),s.querySelector('[data-action="complete"]')?.addEventListener("click",o=>{o.stopPropagation(),t.onComplete?.(t.index)}),s}function nt(e){const t=e.reps||"—",s=e.sets||"—";return`${t} · ${s} sets`}function ot(e=[]){return e.length?`
    <div class="flex gap-1.5 mb-1.5">
      ${e.map(t=>`
        <span class="text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-md bg-slate-800 text-slate-400">${B(t)}</span>
      `).join("")}
    </div>
  `:""}function B(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const it={superset:"Super Set",compound:"Compound",circuit:"Circuit"},lt={superset:"Alternate between exercises with no rest",compound:"Perform back-to-back as one set",circuit:"Rotate through all exercises"};function dt(e,t){const s=document.createElement("article");s.className=`transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=it[e.kind]||e.kind,n=lt[e.kind]||"",r=t.index+1;return s.innerHTML=`
    <div class="border border-brand-500/30 rounded-2xl overflow-hidden bg-brand-500/5">
      ...
      <div class="flex items-center justify-between px-4 py-2 border-b border-brand-500/20">
        ...
        >
          <span class="w-6 h-6 rounded-full border-2 ${t.isCompleted?"bg-brand-500 border-brand-500":"border-slate-600"} flex items-center justify-center transition-colors">
            ${t.isCompleted?'<svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>':""}
          </span>
        </button>
      </div>
      ...
      <div class="px-4 pt-2">
        <div class="bg-brand-500/10 border-l-2 border-brand-500 px-3 py-2 rounded-r-lg">
          <p class="text-xs text-slate-300 leading-relaxed">${U(e.note)}</p>
        </div>
      </div>`:""}

      <!-- Individual exercise cards inside the group -->
      <div class="divide-y divide-slate-800/50">
        ${e.exercises.map((o,i)=>ct(o,i,r,t.isExpanded)).join("")}
      </div>
    </div>
  `,s.querySelectorAll('[data-action="toggle-member"]').forEach(o=>{o.addEventListener("click",()=>{t.onToggle?.(t.index)})}),s.querySelector('[data-action="complete"]')?.addEventListener("click",o=>{o.stopPropagation(),t.onComplete?.(t.index)}),t.isExpanded&&e.exercises.forEach((o,i)=>{const d=s.querySelector(`[data-member-media="${i}"]`),l=o.exercise?.demos||[];d&&l.length>0&&_(d,l)}),s}function ct(e,t,s,a){const n=String.fromCharCode(97+t);return`
    <div class="px-4 py-3">
      <button data-action="toggle-member" class="w-full text-left flex items-center gap-3 active:bg-white/5 transition-colors touch-manipulation rounded-lg -mx-2 px-2 py-1">
        <span class="w-7 h-7 rounded-full bg-slate-700 text-slate-200 text-xs font-bold flex items-center justify-center shrink-0 num">${`${s}${n}`}</span>
        <div class="flex-1 min-w-0">
          <h4 class="font-semibold text-sm text-slate-100 tracking-tight leading-tight">${U(e.name)}</h4>
          <p class="text-xs text-slate-400 mt-0.5 num">${e.reps||"—"} ${e.repUnits||"reps"} · ${e.sets||"—"} sets</p>
        </div>
        <svg class="w-4 h-4 text-slate-500 shrink-0 transition-transform ${a?"rotate-180":""}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
        </svg>
      </button>
      ...
      <div class="mt-3 space-y-3 pl-10">
        ...
        <div class="flex gap-2">
          <div class="flex-1 bg-slate-800/50 rounded-lg p-2 text-center">
            <p class="text-lg font-extrabold text-brand-400 leading-none num tracking-tight">${U(e.reps||"—")}</p>
            <p class="label-meta mt-1">${U(e.repUnits||"reps")}</p>
          </div>
          <div class="flex-1 bg-slate-800/50 rounded-lg p-2 text-center">
            <p class="text-lg font-extrabold text-brand-400 leading-none num tracking-tight">${U(e.sets||"—")}</p>
  ...
  `}function U(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const ie=["Nice work!","Killer moves!","Awesome job!","Crushed it!","You did it!","Beast mode!","On fire!","Way to go!","Strengthened and Conditioned!"];let le=0;function pt(){const e=Date.now();if(e-le<5e3)return;le=e;const t=ie[Math.floor(Math.random()*ie.length)],s=document.createElement("div");s.className="celebration-flash";const a=document.createElement("div");a.className="celebration-text",a.textContent=t,document.body.appendChild(s),document.body.appendChild(a),setTimeout(()=>s.remove(),700),setTimeout(()=>a.remove(),3100)}async function ut(e,t){e.innerHTML=K(`
    <main class="flex-1 px-6 pb-24 flex items-center justify-center">
      <div class="w-8 h-8 border-2 border-brand-500 border-t-transparent rounded-full animate-spin"></div>
    </main>
  `),V(e);try{const s=await He(t);if(!s){xt(e,t);return}Ne(s.id),mt(e,s)}catch(s){ft(e,s)}}function mt(e,t){const s=he(t.id),a=t.resolvedItems.length;let n=-1;e.innerHTML=K(`
...
`),t}async function rs(e,t,s,a,n){const r=ne();if(!r)throw new Error("No API key configured");const i=[{role:"system",content:ss(a)+as(s)},...t.slice(-20),{role:"user",content:e}];let d=await xe(r,i),l=d.choices[0].message,p=0;for(;l.tool_calls&&p<5;){p++;const c=[];for(const u of l.tool_calls){const g=JSON.parse(u.function.arguments),v=await ts(u.function.name,g,s);c.push({role:"tool",tool_call_id:u.id,content:JSON.stringify(v)}),n?.({type:"tool",name:u.function.name,args:g,result:v})}i.push(l),i.push(...c),d=await xe(r,i),l=d.choices[0].message}return l.content||""}async function xe(e,t){const s=await fetch(Wt,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:Zt,messages:t,tools:es,tool_choice:"auto",temperature:.3})});if(!s.ok){const a=await s.text();throw new Error(`OpenAI API error (${s.status}): ${a}`)}return s.json()}function ns(e){const t={id:e.meta.id||"untitled",title:e.meta.title||"Untitled Program"};return e.meta.requirements&&(t.requirements=e.meta.requirements),t.items=e.items,{program:t,newExercises:e.newExercises}}async function os(e){const t=Xt(),s=[];let a=!1;const{exercises:n}=await A(),r=n.map(x=>({id:x.id,name:x.name}));e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>w("/"));const{exercises:t}=await A(),s=e.querySelector('[data-region="count"]');s.textContent=`${t.length} total`;let a=null;const n=l=>{const p=e.querySelector('[data-region="list"]'),c=e.querySelector('[data-region="empty"]');if(l.length===0){p.classList.add("hidden"),c.classList.remove("hidden");return}if(p.classList.remove("hidden"),c.classList.add("hidden"),p.innerHTML=l.map(u=>`
    ...
    `).join(""),p.querySelectorAll('[data-action="expand"]').forEach(u=>{u.addEventListener("click",()=>{if(a=a===u.dataset.id?null:u.dataset.id,n(l),a){const g=p.querySelector(`[data-demo-slot="${a}"]`),v=l.find(k=>k.id===a);g&&v&&v.demos.length>0&&_(g,v.demos)}})}),a){const u=p.querySelector(`[data-demo-slot="${a}"]`),g=l.find(v=>v.id===a);u&&g&&g.demos.length>0&&_(u,g.demos)}};function r(l){return`
