(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();const ge=[];let se=null;function j(e,t){const s=[],a=new RegExp("^"+e.replace(/:([^/]+)/g,(n,r)=>(s.push(r),"([^/]+)"))+"$");ge.push({pattern:e,regex:a,keys:s,handler:t})}function Te(e){se=e}function y(e){window.location.hash=e.startsWith("#")?e:`#${e}`}function oe(){const e=window.location.hash.slice(1)||"/";for(const t of ge){const s=e.match(t.regex);if(s){const a={};t.keys.forEach((n,r)=>{a[n]=decodeURIComponent(s[r+1])}),t.handler(a);return}}se&&se(e)}function Pe(){window.addEventListener("hashchange",oe),window.location.hash?oe():window.location.hash="#/"}const be="action-app:progress",he="action-app:recent-programs",Ie=5;function ae(){try{return JSON.parse(localStorage.getItem(be)||"{}")}catch{return{}}}function ve(e){try{localStorage.setItem(be,JSON.stringify(e))}catch{}}function ye(e){const t=ae();return new Set(t[e]||[])}function Ae(e,t){const s=ae(),a=new Set(s[e]||[]);return a.has(t)?a.delete(t):a.add(t),s[e]=Array.from(a),ve(s),a}function Ne(e){const t=ae();delete t[e],ve(t)}function we(){try{const e=JSON.parse(localStorage.getItem(he)||"[]");return Array.isArray(e)?e:[]}catch{return[]}}function Be(e){if(e)try{const t=we().filter(s=>s.id!==e);t.unshift({id:e,visitedAt:Date.now()}),localStorage.setItem(he,JSON.stringify(t.slice(0,Ie)))}catch{}}const S={workouts:null,exercises:null,plans:null,exerciseMap:null};async function z(){if(S.workouts)return S.workouts;const e=await fetch("./workouts.json");if(!e.ok)throw new Error(`Failed to load workouts.json: ${e.status}`);return S.workouts=await e.json(),S.workouts}async function B(){if(S.exercises)return S.exercises;const e=await fetch("./exercises.json");if(!e.ok)throw new Error(`Failed to load exercises.json: ${e.status}`);return S.exercises=await e.json(),S.exerciseMap=new Map(S.exercises.exercises.map(t=>[t.id,t])),S.exercises}async function ke(){if(S.plans)return S.plans;const e=await fetch("./plans.json");if(!e.ok)throw new Error(`Failed to load plans.json: ${e.status}`);return S.plans=await e.json(),S.plans}async function Ue(e){return(await z()).programs.find(s=>s.id===e)||null}async function He(e){return await B(),S.exerciseMap?.get(e)||null}async function _e(e){const[t]=await Promise.all([Ue(e),B()]);if(!t)return null;const s=r=>{const o=S.exerciseMap.get(r.exerciseId)||null;return{kind:"single",exerciseId:r.exerciseId,exercise:o,name:o?.name||r.exerciseId,reps:r.reps??o?.recommendations?.reps,sets:r.sets??o?.recommendations?.sets,repUnits:r.repUnits??o?.recommendations?.repUnits,note:r.note??o?.recommendations?.note,tags:r.tags||[]}},a=r=>({kind:r.kind,note:r.note,tags:r.tags||[],exercises:r.exercises.map(o=>{const i=S.exerciseMap.get(o.exerciseId)||null;return{exerciseId:o.exerciseId,exercise:i,name:i?.name||o.exerciseId,reps:o.reps??i?.recommendations?.reps,sets:o.sets??i?.recommendations?.sets,repUnits:o.repUnits??i?.recommendations?.repUnits,note:o.note??i?.recommendations?.note}})}),n=(t.items||[]).map(r=>r.kind?a(r):s(r));return{...t,resolvedItems:n}}function ze(){return["localhost","127.0.0.1"].includes(window.location.hostname)}function Re(e){e.innerHTML=`
  ...
  `,e.querySelector('[data-action="create"]')?.addEventListener("click",()=>y("/studio")),e.querySelector('[data-action="search"]')?.addEventListener("click",()=>y("/search")),e.querySelector('[data-action="programs"]')?.addEventListener("click",()=>y("/programs")),e.querySelector('[data-action="exercises"]')?.addEventListener("click",()=>y("/exercises")),Oe(e).catch(t=>console.warn("[recent] skipped",t))}async function Oe(e){const t=we();if(t.length===0)return;const s=e.querySelector('[data-region="recent"]');if(!s)return;const{programs:a}=await z(),n=new Map(a.map(o=>[o.id,o])),r=t.map(o=>({...o,program:n.get(o.id)})).filter(o=>o.program).slice(0,3);r.length!==0&&(s.classList.remove("hidden"),s.innerHTML=`
  ...
  `,s.querySelectorAll("[data-program-id]").forEach(o=>{o.addEventListener("click",()=>y(`/program/${o.dataset.programId}`))}),s.querySelector('[data-action="all-recent"]')?.addEventListener("click",()=>y("/programs")))}function De(e){const t=e.items?.length||e.exercises?.length||0,s=ye(e.id).size,a=t>0?Math.round(s/t*100):0,n=s===0?"Not started":s>=t?"Complete":`${s} of ${t} done`;return`
    <li>
      <button
      ...
      >
        <div class="flex items-center gap-3">
          <div class="flex-1 min-w-0">
            <h3 class="font-semibold tracking-tight truncate">${Fe(e.title)}</h3>
  ...
  `}function Fe(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function Ge(e){e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/"));try{const[t,s]=await Promise.all([z(),ke()]);Je(e,t.programs,s.plans)}catch(t){Ke(e,t)}}function Je(e,t,s){const a=new Map(t.map(r=>[r.id,r])),n=[];for(const r of s)for(const o of r.subPlans||[]){const i=(o.programs||[]).map(d=>a.get(d)).filter(Boolean);i.length!==0&&n.push({category:r.name,title:o.name,description:o.description,programs:i})}e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/")),e.querySelectorAll("[data-program-id]").forEach(r=>{r.addEventListener("click",()=>{y(`/program/${r.dataset.programId}`)})})}function Ye(e){const t=e.items?.length||e.exercises?.length||0;return`
    <li>
      <button
        data-program-id="${e.id}"
        class="w-full card p-4 text-left active:scale-[0.98] transition-transform"
  ...
  `}function $e(e){if(!e)return null;const t=[/youtube\.com\/watch\?v=([^&]+)/,/youtube\.com\/shorts\/([^?&/]+)/,/youtube\.com\/embed\/([^?&/]+)/,/youtu\.be\/([^?&/]+)/];for(const s of t){const a=e.match(s);if(a)return a[1]}return null}function Ve(e,t="hqdefault"){const s=$e(e);return s?`https://i.ytimg.com/vi/${s}/${t}.jpg`:null}function We(e,t={}){const s=$e(e);if(!s)return null;const a=new URLSearchParams({autoplay:"1",rel:"0",modestbranding:"1",playsinline:"1"});return t.startTime&&a.set("start",String(Math.floor(t.startTime))),t.endTime&&a.set("end",String(Math.floor(t.endTime))),`https://www.youtube.com/embed/${s}?${a.toString()}`}function Se(e,t="w_800,q_auto,f_auto"){return!e||!e.includes("cloudinary.com")?e:e.replace("/upload/",`/upload/${t}/`)}function Ze(e){if(!e||!e.type)return"unknown";if(["youtube","tiktok","vimeo"].includes(e.type))return"embed";const t=e.mediaType==="video"||["mp4","webm","mov"].includes(e.format);return e.format==="gif"?"image":t?"video":"image"}function Qe(e,t,s={}){if(!t){e.innerHTML='<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">No media</div>';return}const a=Ze(t),n=s.className||"w-full max-h-[60vh] object-contain rounded-2xl bg-slate-800";switch(e.classList.add("animate-fade-in"),a){case"image":Xe(e,t,n,s.onError);break;case"video":et(e,t,n,s.autoplay,s.onError);break;case"embed":tt(e,t,n,s.onEmbedPlay);break;default:e.innerHTML=`<div class="${n} flex items-center justify-center text-slate-500 text-sm">Unsupported media type</div>`}}function Xe(e,t,s,a){const n=t.type==="cloudinary"?Se(t.url,"w_800,q_auto,f_auto"):t.url;e.innerHTML=`
    <img
      ...
      decoding="async"
    />
  `,a&&e.querySelector("img")?.addEventListener("error",()=>a(),{once:!0})}function et(e,t,s,a=!0,n){const r=t.type==="cloudinary"?Se(t.url,"w_800,q_auto,f_auto"):t.url,o=t.startTime?`#t=${t.startTime}`:"";e.innerHTML=`
    <video
      src="${r}${o}"
      class="${s}"
      ${a?"autoplay":""}
      ...
      preload="metadata"
    ></video>
  `,n&&e.querySelector("video")?.addEventListener("error",()=>n(),{once:!0})}function tt(e,t,s,a){const o=`${(t.url||"").includes("/shorts/")?"aspect-9/16 max-h-[70vh] mx-auto":"aspect-video"} w-full rounded-2xl overflow-hidden bg-slate-900`,i=t.type==="youtube"?Ve(t.url,"hqdefault"):null;e.innerHTML=`
  ...
  `;const d=e.querySelector('[data-action="play-embed"]');d&&d.addEventListener("click",()=>{const l=t.type==="youtube"?We(t.url,{startTime:t.startTime,endTime:t.endTime}):t.url;e.innerHTML=`
        <div class="${o} relative">
          <iframe
            src="${l}"
      ...
      `,a?.()},{once:!0})}function N(e,t,s={}){let a=(t||[]).filter(Boolean);if(a.length===0){e.innerHTML='<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">No demos available</div>';return}a=st(a);let n=ie(s.startIndex??0,a.length);e.innerHTML=`
    <div class="relative">
      <div
        data-region="track"
  ...
  `;const r=e.querySelector('[data-region="track"]'),o=e.querySelector('[data-region="dots"]'),i=e.querySelector('[data-region="caption"]');r.style.scrollbarWidth="none",a.forEach((f,g)=>{const $=document.createElement("div");$.className="shrink-0 w-full snap-center",$.dataset.slideIndex=String(g),r.appendChild($)});const d=new Set,l=new Set,p=f=>{if(d.has(f))return;const g=r.children[f];if(!g)return;const $=a[f];Qe(g,$,{onError:()=>u(f),onEmbedPlay:()=>l.add(f)}),d.add(f)},c=f=>{!l.has(f)||!r.children[f]||(l.delete(f),d.delete(f),p(f))},u=f=>{a.splice(f,1),b()},b=()=>{if(a.length===0){e.innerHTML='<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">No demos available</div>';return}n=ie(n,a.length),N(e,a,{startIndex:n})},h=()=>{o&&(o.innerHTML=a.map((f,g)=>`
      <button
        data-dot-index="${g}"
        aria-label="Go to demo ${g+1}"
        class="p-1.5 -m-1.5 group touch-manipulation"
      >
        <span class="block w-1 h-1 rounded-full transition-colors
          ${g===n?"bg-brand-400":"bg-slate-600 group-hover:bg-slate-500"}"
        ></span>
      </button>
    `).join(""),o.querySelectorAll("[data-dot-index]").forEach(f=>{f.addEventListener("click",()=>E(Number(f.dataset.dotIndex)))}))},w=()=>{i&&(i.textContent=at(a[n]))};function E(f){const g=r.children[f];g&&r.scrollTo({left:g.offsetLeft-r.offsetLeft,behavior:"smooth"})}let x=!1;const k=()=>{x||(x=!0,requestAnimationFrame(()=>{x=!1;const f=r.children[0]?.offsetWidth||1,g=Math.round(r.scrollLeft/f);if(g!==n&&g>=0&&g<a.length){const $=n;n=g,c($),h(),w(),p(n),n+1<a.length&&p(n+1),n-1>=0&&p(n-1)}}))};r.addEventListener("scroll",k,{passive:!0}),p(n),n+1<a.length&&p(n+1),n-1>=0&&p(n-1),requestAnimationFrame(()=>{const f=r.children[n];f&&(r.scrollLeft=f.offsetLeft-r.offsetLeft)}),h(),w()}function ie(e,t){return Math.max(0,Math.min(t-1,e))}function st(e){const t={cloudinary:0,youtube:1,vimeo:2,tiktok:2,url:3,local:4};return[...e].sort((s,a)=>s.isPrimary&&!a.isPrimary?-1:a.isPrimary&&!s.isPrimary?1:(t[s.type]??99)-(t[a.type]??99))}function at(e){const t={cloudinary:e.format==="mp4"?"Original video":"Original",youtube:"YouTube",tiktok:"TikTok",vimeo:"Vimeo",local:"Local",url:"External"}[e.type]||e.type;return e.notes?`${t} · ${e.notes}`:t}function rt(e,t){const s=document.createElement("article");s.className=`card overflow-hidden transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=e.exercise?.demos||[],r=`${t.index+1}. ${e.name}`;if(s.innerHTML=`
      ...
      <div class="px-4 pb-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-slate-800/50 rounded-xl p-3 text-center">
            <p class="text-3xl font-extrabold text-brand-400 leading-none num tracking-tight">${H(e.reps||"—")}</p>
            <p class="label-meta mt-1.5">${H(e.repUnits||"reps")}</p>
          </div>
          <div class="bg-slate-800/50 rounded-xl p-3 text-center">
            <p class="text-3xl font-extrabold text-brand-400 leading-none num tracking-tight">${H(e.sets||"—")}</p>
            <p class="label-meta mt-1.5">sets</p>
          </div>
        </div>
        ${e.note?`
          <div class="bg-brand-500/10 border-l-2 border-brand-500 px-3 py-2.5 rounded-r-lg">
            <p class="text-sm text-slate-300 leading-relaxed">${H(e.note)}</p>
          </div>
        `:""}
      </div>
    </div>
  `,t.isExpanded&&a.length>0){const o=s.querySelector("[data-media-slot]");o&&N(o,a)}return s.querySelector('[data-action="toggle"]')?.addEventListener("click",()=>{t.onToggle?.(t.index)}),s.querySelector('[data-action="complete"]')?.addEventListener("click",o=>{o.stopPropagation(),t.onComplete?.(t.index)}),s}function nt(e){const t=e.reps||"—",s=e.sets||"—";return`${t} · ${s} sets`}function ot(e=[]){return e.length?`
    <div class="flex gap-1.5 mb-1.5">
      ${e.map(t=>`
        <span class="text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-md bg-slate-800 text-slate-400">${H(t)}</span>
      `).join("")}
    </div>
  `:""}function H(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const it={superset:"Super Set",compound:"Compound",circuit:"Circuit"},lt={superset:"Alternate between exercises with no rest",compound:"Perform back-to-back as one set",circuit:"Rotate through all exercises"};function dt(e,t){const s=document.createElement("article");s.className=`card overflow-hidden transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=it[e.kind]||e.kind,n=lt[e.kind]||"",r=e.exercises.length,o=t.index+1,i=e.exercises.map(l=>l.name).join(" + "),d=`${o}. ${i}`;return s.innerHTML=`
    <div class="flex items-stretch">
      <button
        data-action="toggle"
        class="flex-1 min-w-0 px-4 py-4 flex items-center gap-3 text-left active:bg-white/5 transition-colors touch-manipulation"
      >
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-1.5 mb-1.5">
            <span class="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md bg-brand-500/20 text-brand-300">${a}</span>
            ${(e.tags||[]).map(l=>`
              <span class="text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-md bg-slate-800 text-slate-400">${I(l)}</span>
            `).join("")}
          </div>
          <h3 class="font-semibold tracking-tight leading-tight ${t.isCompleted?"line-through text-slate-500":"text-slate-100"}">
            ${I(d)}
          </h3>
          <p class="text-sm text-slate-400 mt-1 num">${r} exercises</p>
        </div>
        <svg class="w-4 h-4 text-slate-500 shrink-0 transition-transform ${t.isExpanded?"rotate-180":""}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
        </svg>
    ...
    <div data-region="content" class="${t.isExpanded?"":"hidden"}">
      <div class="px-4 pb-4 space-y-3">
        ...
        ${e.note?`
          <div class="bg-brand-500/10 border-l-2 border-brand-500 px-3 py-2.5 rounded-r-lg">
            <p class="text-sm text-slate-300 leading-relaxed">${I(e.note)}</p>
          </div>
        `:""}
        <div class="space-y-2">
          ${e.exercises.map((l,p)=>ct(l,p,o)).join("")}
        </div>
      </div>
    </div>
  `,t.isExpanded&&e.exercises.forEach((l,p)=>{const c=s.querySelector(`[data-member-media="${p}"]`),u=l.exercise?.demos||[];c&&u.length>0&&N(c,u)}),s.querySelector('[data-action="toggle"]')?.addEventListener("click",()=>{t.onToggle?.(t.index)}),s.querySelector('[data-action="complete"]')?.addEventListener("click",l=>{l.stopPropagation(),t.onComplete?.(t.index)}),s}function ct(e,t,s){const a=String.fromCharCode(97+t);return`
    <div class="bg-slate-800/40 rounded-xl p-3 space-y-3">
      <div class="flex items-center gap-2">
        <span class="w-6 h-6 rounded-full bg-slate-700 text-slate-300 text-xs font-bold flex items-center justify-center shrink-0 num">${s}${a}</span>
        <h4 class="font-medium text-slate-100 text-sm flex-1 leading-tight tracking-tight">${I(e.name)}</h4>
      </div>
      <div data-member-media="${t}"></div>
      <div class="flex gap-2">
        <div class="flex-1 bg-slate-900/60 rounded-lg p-2 text-center">
          <p class="text-lg font-extrabold text-brand-400 leading-none num tracking-tight">${I(e.reps||"—")}</p>
          <p class="label-meta mt-1">${I(e.repUnits||"reps")}</p>
        </div>
        <div class="flex-1 bg-slate-900/60 rounded-lg p-2 text-center">
          <p class="text-lg font-extrabold text-brand-400 leading-none num tracking-tight">${I(e.sets||"—")}</p>
          <p class="label-meta mt-1">sets</p>
        </div>
      </div>
      ${e.note?`
        <p class="text-xs text-slate-400 leading-relaxed px-1">${I(e.note)}</p>
      `:""}
    </div>
  `}function I(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const le=["Nice work!","Killer moves!","Awesome job!","Crushed it!","You did it!","Beast mode!","On fire!","Way to go!"];let de=0;function pt(){const e=Date.now();if(e-de<5e3)return;de=e;const t=le[Math.floor(Math.random()*le.length)],s=document.createElement("div");s.className="celebration-flash";const a=document.createElement("div");a.className="celebration-text",a.textContent=t,document.body.appendChild(s),document.body.appendChild(a),setTimeout(()=>s.remove(),700),setTimeout(()=>a.remove(),3100)}async function ut(e,t){e.innerHTML=K(`
    <main class="flex-1 px-6 pb-24 flex items-center justify-center">
      <div class="w-8 h-8 border-2 border-brand-500 border-t-transparent rounded-full animate-spin"></div>
    </main>
  `),V(e);try{const s=await _e(t);if(!s){xt(e,t);return}Be(s.id),mt(e,s)}catch(s){ft(e,s)}}function mt(e,t){const s=ye(t.id),a=t.resolvedItems.length;let n=-1;e.innerHTML=K(`
    ...
    <header class="px-6 pt-4 pb-3">
      <h1 class="h-page">${O(t.title)}</h1>
      ${t.requirements?`
        <p class="text-sm text-slate-400 mt-1.5">${O(t.requirements)}</p>
      `:""}
    </header>
  ...
  `,t.title),V(e);const r=e.querySelector('[data-region="items"]'),o=e.querySelector('[data-region="actions"]'),i=()=>{r.innerHTML="",t.resolvedItems.forEach((p,c)=>{const u=document.createElement("li"),b={index:c,isExpanded:c===n,isCompleted:s.has(c),onToggle:w=>{n=n===w?-1:w,l(),n===w&&requestAnimationFrame(()=>{const E=r.querySelector(`[data-item-index="${w}"]`);if(E){const x=window.scrollY+E.getBoundingClientRect().top-130;window.scrollTo({top:Math.max(0,x),behavior:"smooth"})}})},onComplete:w=>{const E=s.size===a,x=Ae(t.id,w);s.clear(),x.forEach(k=>s.add(k)),x.has(w)&&n===w&&(n=-1),l(),!E&&s.size===a&&setTimeout(pt,250)}},h=p.kind==="single"?rt(p,b):dt(p,b);u.appendChild(h),r.appendChild(u)})},d=()=>{const p=e.querySelector('[data-region="progress-bar"]');p&&(p.style.width=`${s.size/a*100}%`);const c=e.querySelector('[data-region="completed-count"]');c&&(c.textContent=String(s.size)),o.classList.toggle("hidden",s.size===0)},l=()=>{i(),d()};l(),e.querySelector('[data-action="reset"]')?.addEventListener("click",()=>{confirm("Reset progress for this program?")&&(Ne(t.id),s.clear(),l())}),e.querySelector('[data-action="share"]')?.addEventListener("click",()=>{const p=window.location.href;navigator.share?navigator.share({title:t.title,text:`Check out: ${t.title}`,url:p}).catch(()=>{}):navigator.clipboard?.writeText(p).then(()=>alert("Link copied!")).catch(()=>prompt("Copy:",p))})}function K(e,t="Program"){return`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        ...
        <span class="text-sm font-medium text-slate-400 truncate">${O(t)}</span>
      </header>
      ${e}
    </div>
  `}function V(e){e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/programs"))}function xt(e,t){e.innerHTML=K(`
    <main class="flex-1 px-6 pt-12 pb-24">
      <div class="card p-6">
  ...
  `),V(e)}function ft(e,t){e.innerHTML=K(`
    <main class="flex-1 px-6 pt-12 pb-24">
      <div class="card p-6">
  ...
  `),V(e)}function O(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function gt(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/")),e.querySelector('[data-action="new-program"]')?.addEventListener("click",()=>y("/studio/program")),e.querySelector('[data-action="new-exercise"]')?.addEventListener("click",()=>y("/studio/exercise")),e.querySelector('[data-action="ai-builder"]')?.addEventListener("click",()=>y("/studio/ai"))}let _=null;async function bt(){if(_)return _;const{exercises:e}=await B();return _=e.map(t=>({id:t.id,name:t.name,hasDemos:(t.demos||[]).length>0,tokens:A(t.name).concat((t.aliases||[]).flatMap(s=>A(s))).concat(A(t.id.replace(/[-_]/g," "))),exercise:t})),_}function ht(e){_&&_.push({id:e.id,name:e.name,hasDemos:(e.demos||[]).length>0,tokens:A(e.name).concat((e.aliases||[]).flatMap(t=>A(t))).concat(A(e.id.replace(/[-_]/g," "))),exercise:e})}async function W(e,t=10){const s=await bt();if(!e||!e.trim())return s.slice(0,t);const a=A(e);return s.map(n=>({...n,score:yt(n.tokens,a)})).filter(n=>n.score>0).sort((n,r)=>r.score-n.score).slice(0,t)}function vt(e,t={}){e.innerHTML=`
    <div class="space-y-3">
      <div class="relative">
  ...
  `;const s=e.querySelector('[data-input="search"]'),a=e.querySelector('[data-region="results"]'),n=e.querySelector('[data-region="empty"]'),r=e.querySelector('[data-action="create-new"]');let o=null;const i=(l,p)=>{if(!p||!p.trim()){a.classList.add("hidden"),n.classList.add("hidden"),r.classList.add("hidden");return}if(r.classList.remove("hidden"),l.length===0){a.classList.add("hidden"),n.classList.remove("hidden");return}n.classList.add("hidden"),a.classList.remove("hidden"),a.innerHTML=l.map(c=>`
      <li>
        ...
        >
          <span class="flex-1 min-w-0">
            <span class="text-sm font-medium text-slate-100 block truncate">${wt(c.name)}</span>
          </span>
          ${c.hasDemos?`
    ...
    `,n()}function a(r,o){const d=(Z.find(p=>p.value===r.type)||Z[0]).fields.includes("startTime"),l=r.type==="youtube"?kt(r.url):null;return`
      <div class="bg-slate-800/40 border border-slate-700/50 rounded-xl p-3 space-y-2.5" data-demo-index="${o}">
        ...
        <div>
          <input data-demo-field="url" data-index="${o}" value="${pe(r.url||"")}" placeholder="https://..." class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-100 placeholder:text-slate-600 focus:outline-hidden focus:border-brand-500 font-mono"/>
        </div>
        ...
        `:""}
        <div>
          <input data-demo-field="notes" data-index="${o}" value="${pe(r.notes||"")}" placeholder="Notes (optional)" class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-100 placeholder:text-slate-600 focus:outline-hidden focus:border-brand-500"/>
        </div>
      </div>
    `}function n(){e.querySelector('[data-action="add-demo"]')?.addEventListener("click",()=>{t.push({type:"youtube",mediaType:"video",format:"youtube",url:"",startTime:0,endTime:0,isPrimary:t.length===0,notes:""}),s()}),e.querySelectorAll('[data-action="remove-demo"]').forEach(r=>{r.addEventListener("click",()=>{const o=+r.dataset.index,i=t[o].isPrimary;t.splice(o,1),i&&t.length>0&&(t[0].isPrimary=!0),s()})}),e.querySelectorAll('input[name="primary-demo"]').forEach(r=>{r.addEventListener("change",()=>{const o=+r.dataset.index;t.forEach((i,d)=>{i.isPrimary=d===o})})}),e.querySelectorAll('[data-demo-field="type"]').forEach(r=>{r.addEventListener("change",()=>{const o=+r.dataset.index;t[o].type=r.value,t[o].format=r.value==="youtube"?"youtube":r.value==="cloudinary"?ce(t[o].url):r.value,t[o].mediaType="video",s()})}),e.querySelectorAll("[data-demo-field]").forEach(r=>{if(r.tagName==="SELECT")return;const o=()=>{const i=+r.dataset.index,d=r.dataset.demoField;d==="startTime"||d==="endTime"?t[i][d]=Number(r.value)||0:t[i][d]=r.value,d==="url"&&t[i].type==="cloudinary"&&(t[i].format=ce(r.value))};r.addEventListener("input",o),r.addEventListener("change",o)})}}function kt(e){if(!e)return null;const t=e.match(/(?:v=|\/shorts\/|youtu\.be\/)([a-zA-Z0-9_-]{11})/);return t?`https://img.youtube.com/vi/${t[1]}/hqdefault.jpg`:null}function ce(e){return e&&/\.(mp4|webm|mov)(\?|$)/i.test(e)?"mp4":"gif"}function pe(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function $t(e,t,s){const{items:a}=t;if(a.length===0){e.innerHTML="";return}e.innerHTML=a.map((n,r)=>n.type==="group"?Et(n,r,t):St(n,r,t)).join(""),Mt(e,t,s)}function St(e,t,s){const a=s.expandedIndex===t,n=e.exerciseNote||"",r=n.length>50?n.substring(0,50)+"…":n,o=(e.tags||[]).map(i=>`<span class="text-[10px] font-medium px-1.5 py-0.5 rounded bg-brand-500/15 text-brand-300">${i}</span>`).join("");return`<li class="card" data-idx="${t}" data-type="single">
...
</li>`}function Et(e,t,s){const a=s.expandedIndex===t,n={superset:"Superset",compound:"Compound",circuit:"Circuit"}[e.kind]||e.kind,r=e.members.map((o,i)=>`
  ...
  ${r}
  ${a?`<div class="border-t border-slate-800" data-region="edit-form" data-idx="${t}"></div>`:""}
</li>`}function Ee(e){return`<button data-action="menu" data-idx="${e}" class="relative z-10 p-3 -mr-2 rounded-lg text-slate-400 hover:text-slate-100 hover:bg-white/10 active:bg-white/20 transition-colors touch-manipulation shrink-0" aria-label="Actions">
  ...
  </button>`}function Lt(e,t,s,a){qt();const n=s.items[t],r=n.type==="group",o=s.items.length,i=t>0?s.items[t-1]:null,d=t<o-1?s.items[t+1]:null;let l="";r?(l+=L("edit","Edit group"),t>0&&(l+=L("move-up","Move up")),t<o-1&&(l+=L("move-down","Move down")),i?.type==="single"&&(l+=L("group-above","Add above to group")),d?.type==="single"&&(l+=L("group-below","Add below to group")),l+=L("ungroup","Ungroup")):(l+=L("edit","Edit"),t>0&&(l+=L("move-up","Move up")),t<o-1&&(l+=L("move-down","Move down")),i?.type==="single"?l+=L("group-above","Group with above"):i?.type==="group"&&(l+=L("group-above","Join group above")),d?.type==="single"?l+=L("group-below","Group with below"):d?.type==="group"&&(l+=L("group-below","Join group below"))),l+=L("remove","Remove","text-red-400");const p=r?n.members.map(u=>u.exerciseName).join(" + "):n.exerciseName||"Item",c=document.createElement("div");c.dataset.region="action-menu",c.className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 animate-fade-in",c.innerHTML=`
  ...
  `,document.body.appendChild(c),c.querySelectorAll("[data-menu-action]").forEach(u=>{u.addEventListener("click",b=>{b.stopPropagation();const h=u.dataset.menuAction;c.remove(),h!=="cancel"&&Ct(h,t,s,a)})}),c.addEventListener("click",u=>{u.target===c&&c.remove()})}function L(e,t,s=""){return`<button data-menu-action="${e}" class="w-full text-left px-5 py-3 text-sm font-medium rounded-xl hover:bg-white/5 active:bg-white/10 transition-colors ${s}">${t}</button>`}function qt(){document.querySelectorAll('[data-region="action-menu"]').forEach(e=>e.remove())}function Ct(e,t,s,a,n){const r=s.items[t],o=t>0?s.items[t-1]:null,i=t<s.items.length-1?s.items[t+1]:null;switch(e){case"edit":a.onEdit?.(t);break;case"move-up":a.onMove?.(t,t-1);break;case"move-down":a.onMove?.(t,t+1);break;case"group-above":o?.type==="group"?a.onJoinGroup?.(t,t-1):r.type==="group"&&o?.type==="single"?a.onAbsorbIntoGroup?.(t,t-1):ue(t,"above",a);break;case"group-below":i?.type==="group"?a.onJoinGroup?.(t,t+1):r.type==="group"&&i?.type==="single"?a.onAbsorbIntoGroup?.(t,t+1):ue(t,"below",a);break;case"ungroup":a.onUngroup?.(t);break;case"remove":a.onRemove?.(t);break}}function ue(e,t,s){const a=document.createElement("div");a.dataset.region="kind-picker",a.className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 animate-fade-in",a.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/studio")),e.querySelector('[data-action="start-fresh"]')?.addEventListener("click",()=>{Q(e,null,!1)}),e.querySelector('[data-action="edit-existing"]')?.addEventListener("click",async()=>{const t=await me();t&&Q(e,t,!0)}),e.querySelector('[data-action="clone-existing"]')?.addEventListener("click",async()=>{const t=await me();t&&Q(e,t,!1)})}function Q(e,t,s){m=Le(),v=-1,e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,Pt(e),It(e),At(e),q(e),zt(e),Ht(),t&&Ut(e,t,s)}function Pt(e){e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/studio"))}function It(e){const t=e.querySelector('[data-field="title"]'),s=e.querySelector('[data-field="requirements"]'),a=e.querySelector('[data-region="id-preview"]'),n=e.querySelector('[data-region="export-section"]');t?.addEventListener("input",()=>{m.meta.title=t.value,m.meta.id=jt(t.value),a.textContent=m.meta.id?`id: ${m.meta.id}`:"",n?.classList.toggle("hidden",!m.meta.title.trim()||m.items.length===0)}),s?.addEventListener("input",()=>{m.meta.requirements=s.value})}function At(e){const t=e.querySelector('[data-region="picker"]');vt(t,{onSelect:s=>{m.items.push({type:"single",exerciseId:s.id,exerciseName:s.name,exerciseNote:s.recommendations?.note||"",reps:s.recommendations?.reps||"",sets:s.recommendations?.sets||"",repUnits:s.recommendations?.repUnits||"reps",note:"",tags:[]}),q(e)},onCreateNew:s=>{_t(e,s)}})}function q(e){const t=e.querySelector('[data-region="timeline"]'),s=e.querySelector('[data-region="empty-timeline"]'),a=e.querySelector('[data-region="item-count"]'),n=e.querySelector('[data-region="export-section"]');if(t){if(a.textContent=`${m.items.length} item${m.items.length!==1?"s":""}`,m.items.length===0){t.classList.add("hidden"),s.classList.remove("hidden"),n?.classList.add("hidden");return}if(t.classList.remove("hidden"),s.classList.add("hidden"),n?.classList.toggle("hidden",!m.meta.title.trim()),$t(t,{items:m.items,expandedIndex:v},{onEdit:r=>{v=v===r?-1:r,q(e)},onRemove:r=>{m.items.splice(r,1),v===r?v=-1:v>r&&v--,q(e)},onMove:(r,o)=>{const[i]=m.items.splice(r,1);m.items.splice(o,0,i),v===r?v=o:r<v&&o>=v?v--:r>v&&o<=v&&v++,q(e)},onGroup:(r,o,i)=>{const d=o==="above"?r-1:r+1,l=Math.min(r,d),p=[m.items[l],m.items[l+1]],c={type:"group",kind:i,note:"",tags:[],members:p};m.items.splice(l,2,c),v=-1,q(e)},onJoinGroup:(r,o)=>{const i=m.items[r];m.items[o].members.push(i),m.items.splice(r,1),v=-1,q(e)},onAbsorbIntoGroup:(r,o)=>{const i=m.items[o],d=m.items[r];o<r?d.members.unshift(i):d.members.push(i),m.items.splice(o,1),v=-1,q(e)},onMemberMove:(r,o,i)=>{const d=m.items[r];if(!d||d.type!=="group")return;const[l]=d.members.splice(o,1);d.members.splice(i,0,l),q(e)},onMemberRemove:(r,o)=>{const i=m.items[r];if(!i||i.type!=="group")return;const[d]=i.members.splice(o,1);if(i.members.length<=1){const l=i.members[0]||d;l.type="single",m.items.splice(r,1,l)}v=-1,q(e)},onUngroup:r=>{const o=m.items[r];if(o.type!=="group")return;const i=o.members.map(d=>({...d,type:"single"}));m.items.splice(r,1,...i),v=-1,q(e)}}),v>=0&&v<m.items.length){const r=t.querySelector(`[data-region="edit-form"][data-idx="${v}"]`);r&&Nt(r,m.items[v],v,e)}}}function Nt(e,t,s,a){const n=["reps","secs","min","yd","rep","reps (each side)","secs (each side)"],r=["warmup","stretch"];e.innerHTML=`
...
Do you still want to create "${d}"?`))return;const u=s.querySelector('[data-exfield="reps"]').value,b=s.querySelector('[data-exfield="sets"]').value,h=s.querySelector('[data-exfield="repUnits"]').value,w=s.querySelector('[data-exfield="note"]').value,E={id:l,name:d,demos:ee.filter(x=>x.url),recommendations:{}};u&&(E.recommendations.reps=u),b&&(E.recommendations.sets=b),h&&h!=="reps"&&(E.recommendations.repUnits=h),w&&(E.recommendations.note=w),m.newExercises.push(E),ht(E),m.items.push({type:"single",exerciseId:l,exerciseName:d,exerciseNote:w||"",reps:u||"",sets:b||"",repUnits:h||"reps",note:"",tags:[]}),s.classList.add("hidden"),q(e)},{once:!0});const i=()=>s.classList.add("hidden");s.querySelector('[data-action="cancel-exercise"]')?.addEventListener("click",i,{once:!0}),s.querySelector('[data-action="close-exercise"]')?.addEventListener("click",i,{once:!0}),s.classList.remove("hidden")}function zt(e){const t=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="export"]')?.addEventListener("click",()=>{Dt(e)}),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>{t?.classList.add("hidden")}),t?.addEventListener("click",s=>{s.target===t&&t.classList.add("hidden")}),e.querySelector('[data-action="preview"]')?.addEventListener("click",()=>{Rt(e)})}function Rt(e){const t=qe(),s=t.items||[],a=`
  ...
  `,n=document.createElement("div");n.innerHTML=a,e.appendChild(n.firstElementChild),e.querySelector('[data-action="close-preview"]')?.addEventListener("click",()=>{e.querySelector(".fixed.inset-0.z-50.bg-slate-950")?.remove()})}function Ot(e,t){if(e.kind)return`<li class="card p-4 space-y-2">
      ...
      <p class="text-sm font-semibold text-slate-100">${C(e.exercises.map(n=>n.exerciseId).join(" + "))}</p>
      <div class="space-y-1.5 pl-3 border-l-2 border-slate-700">
        ${e.exercises.map((n,r)=>`
          <div class="text-xs text-slate-300">${r+1}. ${C(n.exerciseId)} — ${n.reps||"—"} ${n.repUnits||"reps"} · ${n.sets||"—"} sets</div>
        `).join("")}
      </div>
  ...
  </li>`}function Dt(e){const t=e.querySelector('[data-region="export-modal"]'),s=e.querySelector('[data-region="export-content"]');if(!t||!s)return;const a=qe(),n=[];m.newExercises.length>0&&n.push({label:"New Exercises (append to exercises.json → exercises[])",json:m.newExercises}),n.push({label:"Program (append to workouts.json → programs[])",json:a}),s.innerHTML=n.map((r,o)=>`
    <div class="space-y-2">
      <div class="flex items-center justify-between">
...
`)}
`}function as(e){if(e.items.length===0&&!e.meta.title)return`
...
`),t}async function rs(e,t,s,a,n){const r=ne();if(!r)throw new Error("No API key configured");const i=[{role:"system",content:ss(a)+as(s)},...t.slice(-20),{role:"user",content:e}];let d=await fe(r,i),l=d.choices[0].message,p=0;for(;l.tool_calls&&p<5;){p++;const c=[];for(const u of l.tool_calls){const b=JSON.parse(u.function.arguments),h=await ts(u.function.name,b,s);c.push({role:"tool",tool_call_id:u.id,content:JSON.stringify(h)}),n?.({type:"tool",name:u.function.name,args:b,result:h})}i.push(l),i.push(...c),d=await fe(r,i),l=d.choices[0].message}return l.content||""}async function fe(e,t){const s=await fetch(Wt,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:Zt,messages:t,tools:es,tool_choice:"auto",temperature:.3})});if(!s.ok){const a=await s.text();throw new Error(`OpenAI API error (${s.status}): ${a}`)}return s.json()}function ns(e){const t={id:e.meta.id||"untitled",title:e.meta.title||"Untitled Program"};return e.meta.requirements&&(t.requirements=e.meta.requirements),t.items=e.items,{program:t,newExercises:e.newExercises}}async function os(e){const t=Xt(),s=[];let a=!1;const{exercises:n}=await B(),r=n.map(x=>({id:x.id,name:x.name}));e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/studio"));const o=e.querySelector('[data-region="messages"]'),i=e.querySelector('[data-input="message"]'),d=e.querySelector('[data-action="send"]'),l=e.querySelector('[data-action="export"]'),p=e.querySelector('[data-region="settings-modal"]'),c=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="settings"]')?.addEventListener("click",()=>{e.querySelector('[data-input="api-key"]').value=ne(),p.classList.remove("hidden")}),e.querySelector('[data-action="close-settings"]')?.addEventListener("click",()=>p.classList.add("hidden")),e.querySelector('[data-action="save-settings"]')?.addEventListener("click",()=>{const x=e.querySelector('[data-input="api-key"]').value.trim();Qt(x),p.classList.add("hidden")}),p?.addEventListener("click",x=>{x.target===p&&p.classList.add("hidden")}),l?.addEventListener("click",()=>E()),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>c.classList.add("hidden")),c?.addEventListener("click",x=>{x.target===c&&c.classList.add("hidden")}),xe()||p.classList.remove("hidden");async function u(){const x=i.value.trim();if(!x||a)return;if(!xe()){p.classList.remove("hidden");return}i.value="",a=!0,d.disabled=!0,b("user",x),s.push({role:"user",content:x});const k=b("assistant","...");k.dataset.loading="true";try{const f=await rs(x,s,t,r,g=>{g.type==="tool"&&h(g)});k.remove(),b("assistant",f),s.push({role:"assistant",content:f}),w()}catch(f){k.remove(),b("error",f.message)}finally{a=!1,d.disabled=!1,i.focus()}}d?.addEventListener("click",u),i?.addEventListener("keydown",x=>{x.key==="Enter"&&!x.shiftKey&&(x.preventDefault(),u())});function b(x,k){const f=o.querySelector(".text-center.py-8");f&&x!=="error"&&f.remove();const g=document.createElement("div");g.className=x==="user"?"flex justify-end":"flex justify-start";const $=document.createElement("div");return x==="user"?$.className="bg-brand-500/20 text-slate-100 rounded-2xl rounded-br-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":x==="error"?$.className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":$.className="bg-slate-800/60 text-slate-200 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed",$.textContent=k,g.appendChild($),o.appendChild(g),o.scrollTop=o.scrollHeight,g}function h(x){const k=document.createElement("div");k.className="flex justify-start";const f={search_exercises:`🔍 Searching: "${x.args.query}"`,add_exercise:`✓ Added: ${x.args.exerciseId}`,create_exercise:`✓ Created: ${x.args.name}`,remove_exercise:`✗ Removed item at position ${x.args.index}`,group_exercises:`⚡ Grouped as ${x.args.kind}`,set_metadata:`📝 Updated: ${x.args.title||x.args.requirements||"metadata"}`,update_exercise:`✏️ Updated item at position ${x.args.index}`}[x.name]||`🔧 ${x.name}`;k.innerHTML=`<span class="text-[11px] text-slate-500 italic px-2 py-1">${U(f)}</span>`,o.appendChild(k),o.scrollTop=o.scrollHeight}function w(){const x=e.querySelector('[data-region="program-preview"]'),k=e.querySelector('[data-region="program-items"]'),f=e.querySelector('[data-region="item-count"]');if(t.items.length===0){x.classList.add("hidden"),l.classList.add("hidden");return}x.classList.remove("hidden"),l.classList.remove("hidden"),f.textContent=`${t.items.length} item${t.items.length!==1?"s":""}`,k.innerHTML=t.items.map((g,$)=>{if(g.kind)return`<div class="text-xs text-slate-400 pl-2 border-l-2 border-brand-500"><span class="text-brand-300 font-medium">${g.kind}</span>: ${g.exercises.map(je=>je.exerciseId).join(" + ")}</div>`;const T=g.tags?.length?`<span class="text-brand-300">[${g.tags.join(", ")}]</span> `:"";return`<div class="text-xs text-slate-300">${$+1}. ${T}${U(g.exerciseId)} — ${g.reps||"?"} ${g.repUnits||"reps"} × ${g.sets||"?"}</div>`}).join("")}function E(){const{program:x,newExercises:k}=ns(t),f=e.querySelector('[data-region="export-content"]');let g="";if(k.length>0){const T=JSON.stringify(k,null,2);g+=`
        <div class="space-y-2">
          <div class="flex items-center justify-between">
      ...
      `}const $=JSON.stringify(x,null,2);g+=`
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          <p class="text-xs text-slate-400">Program (append to workouts.json)</p>
          <button data-copy="${U($)}" class="text-xs text-brand-400 hover:text-brand-300">Copy</button>
        </div>
        <pre class="bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs text-slate-300 overflow-x-auto max-h-[300px] overflow-y-auto font-mono">${U($)}</pre>
      </div>
    `,f.innerHTML=g,f.querySelectorAll("[data-copy]").forEach(T=>{T.addEventListener("click",()=>{navigator.clipboard?.writeText(T.dataset.copy).then(()=>{T.textContent="✓ Copied",setTimeout(()=>{T.textContent="Copy"},2e3)})})}),c.classList.remove("hidden")}}function U(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function is(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/"));const{exercises:t}=await B(),s=e.querySelector('[data-region="count"]');s.textContent=`${t.length} total`;let a=null;const n=l=>{const p=e.querySelector('[data-region="list"]'),c=e.querySelector('[data-region="empty"]');if(l.length===0){p.classList.add("hidden"),c.classList.remove("hidden");return}if(p.classList.remove("hidden"),c.classList.add("hidden"),p.innerHTML=l.map(u=>`
    ...
    `).join(""),p.querySelectorAll('[data-action="expand"]').forEach(u=>{u.addEventListener("click",()=>{if(a=a===u.dataset.id?null:u.dataset.id,n(l),a){const b=p.querySelector(`[data-demo-slot="${a}"]`),h=l.find(w=>w.id===a);b&&h&&h.demos.length>0&&N(b,h.demos)}})}),a){const u=p.querySelector(`[data-demo-slot="${a}"]`),b=l.find(h=>h.id===a);u&&b&&b.demos.length>0&&N(u,b.demos)}};function r(l){return`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back());const s=await He(t);if(!s){cs(e,t);return}const{programs:a}=await z(),n=ps(a,t);ds(e,s,n)}function ds(e,t,s){const a=t.demos||[],n=t.recommendations||{},r=t.aliases||[];if(e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-4 flex items-center gap-3">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back()),e.querySelectorAll("[data-program-id]").forEach(o=>{o.addEventListener("click",()=>y(`/program/${o.dataset.programId}`))}),a.length>0){const o=e.querySelector('[data-region="demos"]');N(o,a)}}function cs(e,t){e.innerHTML=`
  ...
  `}function ps(e,t){return e.filter(s=>{for(const a of s.items||[]){if(a.exerciseId===t)return!0;if(a.exercises){for(const n of a.exercises)if(n.exerciseId===t)return!0}}return!1})}function P(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function us(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>y("/"));const[{programs:t},{plans:s}]=await Promise.all([z(),ke()]),a=t.map(c=>({id:c.id,title:c.title,requirements:c.requirements||"",itemCount:c.items?.length||c.exercises?.length||0,tokens:J(c.title).concat(J(c.requirements||"")).concat(J(c.id.replace(/[-_]/g," "))),program:c})),n=new Map;for(const c of s)for(const u of c.subPlans||[])for(const b of u.programs||[])n.set(b,`${c.name} · ${u.name}`);const r=e.querySelector('[data-region="results"]'),o=e.querySelector('[data-region="empty"]'),i=e.querySelector('[data-region="initial"]');function d(c){if(c===null){r.classList.add("hidden"),o.classList.add("hidden"),i.classList.remove("hidden");return}if(i.classList.add("hidden"),c.length===0){r.classList.add("hidden"),o.classList.remove("hidden");return}o.classList.add("hidden"),r.classList.remove("hidden"),r.innerHTML=c.map(u=>ms(u,n.get(u.id))).join(""),r.querySelectorAll("[data-program-id]").forEach(u=>{u.addEventListener("click",()=>y(`/program/${u.dataset.programId}`))})}const l=e.querySelector('[data-input="search"]');let p=null;l?.addEventListener("input",()=>{clearTimeout(p),p=setTimeout(()=>{const c=l.value.trim();if(!c){d(null);return}const u=J(c),b=a.map(h=>({...h,score:xs(h.tokens,u)})).filter(h=>h.score>0).sort((h,w)=>w.score-h.score);d(b)},150)}),d(null)}function ms(e,t){return`
    <button
      data-program-id="${e.id}"
      class="w-full card p-4 text-left active:scale-[0.98] transition-transform"
    >
  ...
  `}function J(e){return e?e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>0):[]}function xs(e,t){let s=0;for(const a of t){let n=0;for(const r of e)r===a?n=Math.max(n,10):r.startsWith(a)?n=Math.max(n,7):r.includes(a)&&(n=Math.max(n,4));if(n===0)return 0;s+=n}return s}function te(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const M=document.getElementById("app");j("/",()=>Re(M));j("/programs",()=>Ge(M));j("/program/:id",({id:e})=>ut(M,e));j("/exercises",()=>is(M));j("/exercise/:id",({id:e})=>ls(M,e));j("/search",()=>us(M));const fs=["localhost","127.0.0.1"].includes(window.location.hostname);fs&&(j("/studio",()=>gt(M)),j("/studio/program",()=>Tt(M)),j("/studio/exercise",()=>Ft(M)),j("/studio/ai",()=>os(M)));Te(e=>{M.innerHTML=`
    <div class="flex-1 flex flex-col items-center justify-center min-h-screen px-6 text-center">
      <p class="text-6xl mb-4">🤔</p>
  ...
  `});Pe();console.log("🚀 Action App V2 ready");
