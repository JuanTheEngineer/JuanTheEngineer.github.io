(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();const ge=[];let se=null;function j(e,t){const s=[],a=new RegExp("^"+e.replace(/:([^/]+)/g,(n,r)=>(s.push(r),"([^/]+)"))+"$");ge.push({pattern:e,regex:a,keys:s,handler:t})}function Te(e){se=e}function h(e){window.history.pushState(null,"",e),ae()}function ae(){const e=window.location.pathname||"/";for(const t of ge){const s=e.match(t.regex);if(s){const a={};t.keys.forEach((n,r)=>{a[n]=decodeURIComponent(s[r+1])}),t.handler(a);return}}se&&se(e)}function Pe(){if(window.addEventListener("popstate",ae),document.addEventListener("click",e=>{const t=e.target.closest("a[href]");if(!t)return;const s=t.getAttribute("href");s&&s.startsWith("/")&&!s.startsWith("//")&&(e.preventDefault(),h(s))}),window.location.hash&&window.location.hash.startsWith("#/")){const e=window.location.hash.slice(1);window.history.replaceState(null,"",e)}ae()}const be="action-app:progress",he="action-app:recent-programs",Ie=5;function re(){try{return JSON.parse(localStorage.getItem(be)||"{}")}catch{return{}}}function ve(e){try{localStorage.setItem(be,JSON.stringify(e))}catch{}}function ye(e){const t=re();return new Set(t[e]||[])}function Ae(e,t){const s=re(),a=new Set(s[e]||[]);return a.has(t)?a.delete(t):a.add(t),s[e]=Array.from(a),ve(s),a}function Be(e){const t=re();delete t[e],ve(t)}function we(){try{const e=JSON.parse(localStorage.getItem(he)||"[]");return Array.isArray(e)?e:[]}catch{return[]}}function Ne(e){if(e)try{const t=we().filter(s=>s.id!==e);t.unshift({id:e,visitedAt:Date.now()}),localStorage.setItem(he,JSON.stringify(t.slice(0,Ie)))}catch{}}const w={workouts:null,exercises:null,plans:null,exerciseMap:null};async function _(){if(w.workouts)return w.workouts;const e=await fetch("/workouts.json");if(!e.ok)throw new Error(`Failed to load workouts.json: ${e.status}`);return w.workouts=await e.json(),w.workouts}async function A(){if(w.exercises)return w.exercises;const e=await fetch("/exercises.json");if(!e.ok)throw new Error(`Failed to load exercises.json: ${e.status}`);return w.exercises=await e.json(),w.exerciseMap=new Map(w.exercises.exercises.map(t=>[t.id,t])),w.exercises}async function ke(){if(w.plans)return w.plans;const e=await fetch("/plans.json");if(!e.ok)throw new Error(`Failed to load plans.json: ${e.status}`);return w.plans=await e.json(),w.plans}async function Ue(e){return(await _()).programs.find(s=>s.id===e)||null}async function He(e){return await A(),w.exerciseMap?.get(e)||null}async function _e(e){const[t]=await Promise.all([Ue(e),A()]);if(!t)return null;const s=r=>{const o=w.exerciseMap.get(r.exerciseId)||null;return{kind:"single",exerciseId:r.exerciseId,exercise:o,name:o?.name||r.exerciseId,reps:r.reps??o?.recommendations?.reps,sets:r.sets??o?.recommendations?.sets,repUnits:r.repUnits??o?.recommendations?.repUnits,note:r.note??o?.recommendations?.note,tags:r.tags||[]}},a=r=>({kind:r.kind,note:r.note,tags:r.tags||[],exercises:r.exercises.map(o=>{const i=w.exerciseMap.get(o.exerciseId)||null;return{exerciseId:o.exerciseId,exercise:i,name:i?.name||o.exerciseId,reps:o.reps??i?.recommendations?.reps,sets:o.sets??i?.recommendations?.sets,repUnits:o.repUnits??i?.recommendations?.repUnits,note:o.note??i?.recommendations?.note}})}),n=(t.items||[]).map(r=>r.kind?a(r):s(r));return{...t,resolvedItems:n}}function ze(){return["localhost","127.0.0.1"].includes(window.location.hostname)}function Re(e){e.innerHTML=`
  ...
  `;const r=e.querySelector('[data-region="track"]'),o=e.querySelector('[data-region="dots"]'),i=e.querySelector('[data-region="caption"]');r.style.scrollbarWidth="none";const d=new Set;t.forEach((p,x)=>{const g=document.createElement("div");g.className="shrink-0 w-full snap-center",ie(g,p,{onError:()=>{g.innerHTML=`<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">Couldn't load</div>`},onEmbedPlay:()=>d.add(x)}),r.appendChild(g)});const l=()=>{o&&(o.innerHTML=t.map((p,x)=>`
        <button data-dot-index="${x}" aria-label="Demo ${x+1}" class="p-1.5 -m-1.5 group touch-manipulation">
          <span class="block w-1 h-1 rounded-full transition-colors ${x===n?"bg-brand-400":"bg-slate-600 group-hover:bg-slate-500"}"></span>
        </button>`).join(""),o.querySelectorAll("[data-dot-index]").forEach(p=>{p.addEventListener("click",()=>{a(!0);const x=Number(p.dataset.dotIndex),g=r.children[x];g&&r.scrollTo({left:g.offsetLeft-r.offsetLeft,behavior:"smooth"})})}))},u=()=>{if(!i)return;const p=t[n],x=nt(p),g=p.url;g&&(p.type==="youtube"||p.type==="tiktok"||p.type==="vimeo")?i.innerHTML=`<a href="${g}" target="_blank" rel="noopener" class="hover:text-brand-400 transition-colors">${ot(x)} ↗</a>`:i.textContent=x};let c=!1;r.addEventListener("scroll",()=>{c||(c=!0,requestAnimationFrame(()=>{c=!1;const p=r.children[0]?.offsetWidth||1,x=Math.round(r.scrollLeft/p);if(x!==n&&x>=0&&x<t.length){if(a(!0),d.has(n)){const g=r.children[n];g&&(d.delete(n),ie(g,t[n],{onError:()=>{},onEmbedPlay:()=>d.add(n)}))}n=x,l(),u()}}))},{passive:!0}),requestAnimationFrame(()=>{const p=r.children[n];p&&(r.scrollLeft=p.offsetLeft-r.offsetLeft)}),l(),u()}function at(e,t){return Math.max(0,Math.min(t-1,e))}function rt(e){const t={cloudinary:0,youtube:1,vimeo:2,tiktok:2,url:3,local:4};return[...e].sort((s,a)=>s.isPrimary&&!a.isPrimary?-1:a.isPrimary&&!s.isPrimary?1:(t[s.type]??99)-(t[a.type]??99))}function nt(e){const t={cloudinary:e.format==="mp4"?"Video":"Demo",youtube:"YouTube",tiktok:"TikTok",vimeo:"Vimeo",local:"Demo",url:"External"}[e.type]||e.type,s=e.metadata?.channel;return s?`${t} · ${s}`:e.notes?`${t} · ${e.notes}`:t}function ot(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function it(e,t){const s=document.createElement("article");s.className=`card overflow-hidden transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=e.exercise?.demos||[],r=`${t.index+1}. ${e.name}`;if(s.innerHTML=`
  ...
  `,t.isExpanded&&a.length>0){const o=s.querySelector("[data-media-slot]");o&&H(o,a)}return s.querySelector('[data-action="toggle"]')?.addEventListener("click",()=>{t.onToggle?.(t.index)}),s.querySelector('[data-action="complete"]')?.addEventListener("click",o=>{o.stopPropagation(),t.onComplete?.(t.index)}),s}function lt(e){const t=e.reps||"—",s=e.sets||"—";return`${t} · ${s} sets`}function dt(e=[]){return e.length?`
  ...
  `:""}function N(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const ct={superset:"Super Set",compound:"Compound",circuit:"Circuit"};function pt(e,t){const s=document.createElement("article");s.className=`transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=ct[e.kind]||e.kind,n=t.index+1,r=e.exercises.map((o,i)=>{const d=String.fromCharCode(97+i),l=`${n}${d}. ${z(o.name)}`,u=i===0,c=i===e.exercises.length-1;return`
  ...
  `;const s=e.querySelector('[data-input="search"]'),a=e.querySelector('[data-region="results"]'),n=e.querySelector('[data-region="empty"]'),r=e.querySelector('[data-action="create-new"]');let o=null;const i=(l,u)=>{if(!u||!u.trim()){a.classList.add("hidden"),n.classList.add("hidden"),r.classList.add("hidden");return}if(r.classList.remove("hidden"),l.length===0){a.classList.add("hidden"),n.classList.remove("hidden");return}n.classList.add("hidden"),a.classList.remove("hidden"),a.innerHTML=l.map(c=>`
      <li>
        ...
        >
          <span class="flex-1 min-w-0">
            <span class="text-sm font-medium text-slate-100 block truncate">${kt(c.name)}</span>
          </span>
          ${c.hasDemos?`
    ...
    `).join(""),a.querySelectorAll("[data-exercise-id]").forEach(c=>{c.addEventListener("click",()=>{const p=l.find(x=>x.id===c.dataset.exerciseId);p&&t.onSelect?.(p.exercise)})})},d=async()=>{const l=s.value,u=await K(l);i(u,l)};s.addEventListener("input",()=>{clearTimeout(o),o=setTimeout(d,150)}),i([],""),r?.addEventListener("click",()=>t.onCreateNew?.(s.value.trim()))}function I(e){return e?e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>0):[]}function wt(e,t){let s=0;for(const a of t){let n=0;for(const r of e)r===a?n=Math.max(n,10):r.startsWith(a)?n=Math.max(n,7):r.includes(a)&&(n=Math.max(n,4));if(n===0)return 0;s+=n}return s}function kt(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const Z=[{value:"youtube",label:"YouTube",fields:["url","startTime","endTime","notes"]},{value:"cloudinary",label:"Cloudinary",fields:["url","startTime","endTime","notes"]},{value:"local",label:"Local file",fields:["url","notes"]},{value:"url",label:"URL (external)",fields:["url","notes"]},{value:"tiktok",label:"TikTok",fields:["url","notes"]},{value:"vimeo",label:"Vimeo",fields:["url","startTime","endTime","notes"]}];function ne(e,t){s();function s(){e.innerHTML=`
      <div class="space-y-3">
    ...
    `,n()}function a(r,o){const d=(Z.find(u=>u.value===r.type)||Z[0]).fields.includes("startTime"),l=r.type==="youtube"?$t(r.url):null;return`
  ...
  `,document.body.appendChild(c),c.querySelectorAll("[data-menu-action]").forEach(p=>{p.addEventListener("click",x=>{x.stopPropagation();const g=p.dataset.menuAction;c.remove(),g!=="cancel"&&Mt(g,t,s,a)})}),c.addEventListener("click",p=>{p.target===c&&c.remove()})}function S(e,t,s=""){return`<button data-menu-action="${e}" class="w-full text-left px-5 py-3 text-sm font-medium rounded-xl hover:bg-white/5 active:bg-white/10 transition-colors ${s}">${t}</button>`}function Ct(){document.querySelectorAll('[data-region="action-menu"]').forEach(e=>e.remove())}function Mt(e,t,s,a,n){const r=s.items[t],o=t>0?s.items[t-1]:null,i=t<s.items.length-1?s.items[t+1]:null;switch(e){case"edit":a.onEdit?.(t);break;case"move-up":a.onMove?.(t,t-1);break;case"move-down":a.onMove?.(t,t+1);break;case"group-above":o?.type==="group"?a.onJoinGroup?.(t,t-1):r.type==="group"&&o?.type==="single"?a.onAbsorbIntoGroup?.(t,t-1):ue(t,"above",a);break;case"group-below":i?.type==="group"?a.onJoinGroup?.(t,t+1):r.type==="group"&&i?.type==="single"?a.onAbsorbIntoGroup?.(t,t+1):ue(t,"below",a);break;case"ungroup":a.onUngroup?.(t);break;case"remove":a.onRemove?.(t);break}}function ue(e,t,s){const a=document.createElement("div");a.dataset.region="kind-picker",a.className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 animate-fade-in",a.innerHTML=`
...
Do you still want to create "${d}"?`))return;const p=s.querySelector('[data-exfield="reps"]').value,x=s.querySelector('[data-exfield="sets"]').value,g=s.querySelector('[data-exfield="repUnits"]').value,y=s.querySelector('[data-exfield="note"]').value,L={id:l,name:d,demos:ee.filter(f=>f.url),recommendations:{}};p&&(L.recommendations.reps=p),x&&(L.recommendations.sets=x),g&&g!=="reps"&&(L.recommendations.repUnits=g),y&&(L.recommendations.note=y),m.newExercises.push(L),vt(L),m.items.push({type:"single",exerciseId:l,exerciseName:d,exerciseNote:y||"",reps:p||"",sets:x||"",repUnits:g||"reps",note:"",tags:[]}),s.classList.add("hidden"),E(e)},{once:!0});const i=()=>s.classList.add("hidden");s.querySelector('[data-action="cancel-exercise"]')?.addEventListener("click",i,{once:!0}),s.querySelector('[data-action="close-exercise"]')?.addEventListener("click",i,{once:!0}),s.classList.remove("hidden")}function Rt(e){const t=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="export"]')?.addEventListener("click",()=>{Ft(e)}),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>{t?.classList.add("hidden")}),t?.addEventListener("click",s=>{s.target===t&&t.classList.add("hidden")}),e.querySelector('[data-action="preview"]')?.addEventListener("click",()=>{Ot(e)})}function Ot(e){const t=qe(),s=t.items||[],a=`
  ...
  `,n=document.createElement("div");n.innerHTML=a,e.appendChild(n.firstElementChild),e.querySelector('[data-action="close-preview"]')?.addEventListener("click",()=>{e.querySelector(".fixed.inset-0.z-50.bg-slate-950")?.remove()})}function Dt(e,t){if(e.kind)return`<li class="card p-4 space-y-2">
...
`)}
`}function rs(e){if(e.items.length===0&&!e.meta.title)return`
...
`),t}async function ns(e,t,s,a,n){const r=oe();if(!r)throw new Error("No API key configured");const i=[{role:"system",content:as(a)+rs(s)},...t.slice(-20),{role:"user",content:e}];let d=await fe(r,i),l=d.choices[0].message,u=0;for(;l.tool_calls&&u<5;){u++;const c=[];for(const p of l.tool_calls){const x=JSON.parse(p.function.arguments),g=await ss(p.function.name,x,s);c.push({role:"tool",tool_call_id:p.id,content:JSON.stringify(g)}),n?.({type:"tool",name:p.function.name,args:x,result:g})}i.push(l),i.push(...c),d=await fe(r,i),l=d.choices[0].message}return l.content||""}async function fe(e,t){const s=await fetch(Zt,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:Qt,messages:t,tools:ts,tool_choice:"auto",temperature:.3})});if(!s.ok){const a=await s.text();throw new Error(`OpenAI API error (${s.status}): ${a}`)}return s.json()}function os(e){const t={id:e.meta.id||"untitled",title:e.meta.title||"Untitled Program"};return e.meta.requirements&&(t.requirements=e.meta.requirements),t.items=e.items,{program:t,newExercises:e.newExercises}}async function is(e){const t=es(),s=[];let a=!1;const{exercises:n}=await A(),r=n.map(f=>({id:f.id,name:f.name}));e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>h("/studio"));const o=e.querySelector('[data-region="messages"]'),i=e.querySelector('[data-input="message"]'),d=e.querySelector('[data-action="send"]'),l=e.querySelector('[data-action="export"]'),u=e.querySelector('[data-region="settings-modal"]'),c=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="settings"]')?.addEventListener("click",()=>{e.querySelector('[data-input="api-key"]').value=oe(),u.classList.remove("hidden")}),e.querySelector('[data-action="close-settings"]')?.addEventListener("click",()=>u.classList.add("hidden")),e.querySelector('[data-action="save-settings"]')?.addEventListener("click",()=>{const f=e.querySelector('[data-input="api-key"]').value.trim();Xt(f),u.classList.add("hidden")}),u?.addEventListener("click",f=>{f.target===u&&u.classList.add("hidden")}),l?.addEventListener("click",()=>L()),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>c.classList.add("hidden")),c?.addEventListener("click",f=>{f.target===c&&c.classList.add("hidden")}),xe()||u.classList.remove("hidden");async function p(){const f=i.value.trim();if(!f||a)return;if(!xe()){u.classList.remove("hidden");return}i.value="",a=!0,d.disabled=!0,x("user",f),s.push({role:"user",content:f});const k=x("assistant","...");k.dataset.loading="true";try{const $=await ns(f,s,t,r,v=>{v.type==="tool"&&g(v)});k.remove(),x("assistant",$),s.push({role:"assistant",content:$}),y()}catch($){k.remove(),x("error",$.message)}finally{a=!1,d.disabled=!1,i.focus()}}d?.addEventListener("click",p),i?.addEventListener("keydown",f=>{f.key==="Enter"&&!f.shiftKey&&(f.preventDefault(),p())});function x(f,k){const $=o.querySelector(".text-center.py-8");$&&f!=="error"&&$.remove();const v=document.createElement("div");v.className=f==="user"?"flex justify-end":"flex justify-start";const M=document.createElement("div");return f==="user"?M.className="bg-brand-500/20 text-slate-100 rounded-2xl rounded-br-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":f==="error"?M.className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":M.className="bg-slate-800/60 text-slate-200 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed",M.textContent=k,v.appendChild(M),o.appendChild(v),o.scrollTop=o.scrollHeight,v}function g(f){const k=document.createElement("div");k.className="flex justify-start";const $={search_exercises:`🔍 Searching: "${f.args.query}"`,add_exercise:`✓ Added: ${f.args.exerciseId}`,create_exercise:`✓ Created: ${f.args.name}`,remove_exercise:`✗ Removed item at position ${f.args.index}`,group_exercises:`⚡ Grouped as ${f.args.kind}`,set_metadata:`📝 Updated: ${f.args.title||f.args.requirements||"metadata"}`,update_exercise:`✏️ Updated item at position ${f.args.index}`}[f.name]||`🔧 ${f.name}`;k.innerHTML=`<span class="text-[11px] text-slate-500 italic px-2 py-1">${B($)}</span>`,o.appendChild(k),o.scrollTop=o.scrollHeight}function y(){const f=e.querySelector('[data-region="program-preview"]'),k=e.querySelector('[data-region="program-items"]'),$=e.querySelector('[data-region="item-count"]');if(t.items.length===0){f.classList.add("hidden"),l.classList.add("hidden");return}f.classList.remove("hidden"),l.classList.remove("hidden"),$.textContent=`${t.items.length} item${t.items.length!==1?"s":""}`,k.innerHTML=t.items.map((v,M)=>{if(v.kind)return`<div class="text-xs text-slate-400 pl-2 border-l-2 border-brand-500"><span class="text-brand-300 font-medium">${v.kind}</span>: ${v.exercises.map(je=>je.exerciseId).join(" + ")}</div>`;const T=v.tags?.length?`<span class="text-brand-300">[${v.tags.join(", ")}]</span> `:"";return`<div class="text-xs text-slate-300">${M+1}. ${T}${B(v.exerciseId)} — ${v.reps||"?"} ${v.repUnits||"reps"} × ${v.sets||"?"}</div>`}).join("")}function L(){const{program:f,newExercises:k}=os(t),$=e.querySelector('[data-region="export-content"]');let v="";if(k.length>0){const T=JSON.stringify(k,null,2);v+=`
        <div class="space-y-2">
          <div class="flex items-center justify-between">
      ...
      `}const M=JSON.stringify(f,null,2);v+=`
      <div class="space-y-2">
        <div class="flex items-center justify-between">
    ...
    `,$.innerHTML=v,$.querySelectorAll("[data-copy]").forEach(T=>{T.addEventListener("click",()=>{navigator.clipboard?.writeText(T.dataset.copy).then(()=>{T.textContent="✓ Copied",setTimeout(()=>{T.textContent="Copy"},2e3)})})}),c.classList.remove("hidden")}}function B(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function ls(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>h("/"));const{exercises:t}=await A(),s=e.querySelector('[data-region="count"]');s.textContent=`${t.length} total`;let a=null;const n=l=>{const u=e.querySelector('[data-region="list"]'),c=e.querySelector('[data-region="empty"]');if(l.length===0){u.classList.add("hidden"),c.classList.remove("hidden");return}if(u.classList.remove("hidden"),c.classList.add("hidden"),u.innerHTML=l.map(p=>`
    ...
    `).join(""),u.querySelectorAll('[data-action="expand"]').forEach(p=>{p.addEventListener("click",()=>{if(a=a===p.dataset.id?null:p.dataset.id,n(l),a){const x=u.querySelector(`[data-demo-slot="${a}"]`),g=l.find(y=>y.id===a);x&&g&&g.demos.length>0&&H(x,g.demos)}})}),a){const p=u.querySelector(`[data-demo-slot="${a}"]`),x=l.find(g=>g.id===a);p&&x&&x.demos.length>0&&H(p,x.demos)}};function r(l){return`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back());const s=await He(t);if(!s){ps(e,t);return}const{programs:a}=await _(),n=us(a,t);cs(e,s,n)}function cs(e,t,s){const a=t.demos||[],n=t.recommendations||{},r=t.aliases||[];if(e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-4 flex items-center gap-3">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back()),e.querySelectorAll("[data-program-id]").forEach(o=>{o.addEventListener("click",()=>h(`/program/${o.dataset.programId}`))}),a.length>0){const o=e.querySelector('[data-region="demos"]');H(o,a)}}function ps(e,t){e.innerHTML=`
  ...
  `}function us(e,t){return e.filter(s=>{for(const a of s.items||[]){if(a.exerciseId===t)return!0;if(a.exercises){for(const n of a.exercises)if(n.exerciseId===t)return!0}}return!1})}function P(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function ms(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>h("/"));const[{programs:t},{plans:s}]=await Promise.all([_(),ke()]),a=t.map(c=>({id:c.id,title:c.title,requirements:c.requirements||"",itemCount:c.items?.length||c.exercises?.length||0,tokens:G(c.title).concat(G(c.requirements||"")).concat(G(c.id.replace(/[-_]/g," "))),program:c})),n=new Map;for(const c of s)for(const p of c.subPlans||[])for(const x of p.programs||[])n.set(x,`${c.name} · ${p.name}`);const r=e.querySelector('[data-region="results"]'),o=e.querySelector('[data-region="empty"]'),i=e.querySelector('[data-region="initial"]');function d(c){if(c===null){r.classList.add("hidden"),o.classList.add("hidden"),i.classList.remove("hidden");return}if(i.classList.add("hidden"),c.length===0){r.classList.add("hidden"),o.classList.remove("hidden");return}o.classList.add("hidden"),r.classList.remove("hidden"),r.innerHTML=c.map(p=>xs(p,n.get(p.id))).join(""),r.querySelectorAll("[data-program-id]").forEach(p=>{p.addEventListener("click",()=>h(`/program/${p.dataset.programId}`))})}const l=e.querySelector('[data-input="search"]');let u=null;l?.addEventListener("input",()=>{clearTimeout(u),u=setTimeout(()=>{const c=l.value.trim();if(!c){d(null);return}const p=G(c),x=a.map(g=>({...g,score:fs(g.tokens,p)})).filter(g=>g.score>0).sort((g,y)=>y.score-g.score);d(x)},150)}),d(null)}function xs(e,t){return`
    <button
      data-program-id="${e.id}"
      class="w-full card p-4 text-left active:scale-[0.98] transition-transform"
    >
  ...
  `}function G(e){return e?e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>0):[]}function fs(e,t){let s=0;for(const a of t){let n=0;for(const r of e)r===a?n=Math.max(n,10):r.startsWith(a)?n=Math.max(n,7):r.includes(a)&&(n=Math.max(n,4));if(n===0)return 0;s+=n}return s}function te(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const C=document.getElementById("app");j("/",()=>Re(C));j("/programs",()=>Je(C));j("/program/:id",({id:e})=>mt(C,e));j("/exercises",()=>ls(C));j("/exercise/:id",({id:e})=>ds(C,e));j("/search",()=>ms(C));const gs=["localhost","127.0.0.1"].includes(window.location.hostname);gs&&(j("/studio",()=>bt(C)),j("/studio/program",()=>Pt(C)),j("/studio/exercise",()=>Jt(C)),j("/studio/ai",()=>is(C)));Te(e=>{C.innerHTML=`
    <div class="flex-1 flex flex-col items-center justify-center min-h-screen px-6 text-center">
      <p class="text-6xl mb-4">🤔</p>
