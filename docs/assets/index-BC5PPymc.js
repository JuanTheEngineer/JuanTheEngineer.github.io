(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();const ge=[];let se=null;function j(e,t){const s=[],a=new RegExp("^"+e.replace(/:([^/]+)/g,(n,r)=>(s.push(r),"([^/]+)"))+"$");ge.push({pattern:e,regex:a,keys:s,handler:t})}function Te(e){se=e}function v(e){window.location.hash=e.startsWith("#")?e:`#${e}`}function oe(){const e=window.location.hash.slice(1)||"/";for(const t of ge){const s=e.match(t.regex);if(s){const a={};t.keys.forEach((n,r)=>{a[n]=decodeURIComponent(s[r+1])}),t.handler(a);return}}se&&se(e)}function Pe(){window.addEventListener("hashchange",oe),window.location.hash?oe():window.location.hash="#/"}const be="action-app:progress",ve="action-app:recent-programs",Ie=5;function ae(){try{return JSON.parse(localStorage.getItem(be)||"{}")}catch{return{}}}function he(e){try{localStorage.setItem(be,JSON.stringify(e))}catch{}}function ye(e){const t=ae();return new Set(t[e]||[])}function Ae(e,t){const s=ae(),a=new Set(s[e]||[]);return a.has(t)?a.delete(t):a.add(t),s[e]=Array.from(a),he(s),a}function Be(e){const t=ae();delete t[e],he(t)}function we(){try{const e=JSON.parse(localStorage.getItem(ve)||"[]");return Array.isArray(e)?e:[]}catch{return[]}}function Ne(e){if(e)try{const t=we().filter(s=>s.id!==e);t.unshift({id:e,visitedAt:Date.now()}),localStorage.setItem(ve,JSON.stringify(t.slice(0,Ie)))}catch{}}const w={workouts:null,exercises:null,plans:null,exerciseMap:null};async function _(){if(w.workouts)return w.workouts;const e=await fetch("./workouts.json");if(!e.ok)throw new Error(`Failed to load workouts.json: ${e.status}`);return w.workouts=await e.json(),w.workouts}async function A(){if(w.exercises)return w.exercises;const e=await fetch("./exercises.json");if(!e.ok)throw new Error(`Failed to load exercises.json: ${e.status}`);return w.exercises=await e.json(),w.exerciseMap=new Map(w.exercises.exercises.map(t=>[t.id,t])),w.exercises}async function ke(){if(w.plans)return w.plans;const e=await fetch("./plans.json");if(!e.ok)throw new Error(`Failed to load plans.json: ${e.status}`);return w.plans=await e.json(),w.plans}async function Ue(e){return(await _()).programs.find(s=>s.id===e)||null}async function He(e){return await A(),w.exerciseMap?.get(e)||null}async function _e(e){const[t]=await Promise.all([Ue(e),A()]);if(!t)return null;const s=r=>{const o=w.exerciseMap.get(r.exerciseId)||null;return{kind:"single",exerciseId:r.exerciseId,exercise:o,name:o?.name||r.exerciseId,reps:r.reps??o?.recommendations?.reps,sets:r.sets??o?.recommendations?.sets,repUnits:r.repUnits??o?.recommendations?.repUnits,note:r.note??o?.recommendations?.note,tags:r.tags||[]}},a=r=>({kind:r.kind,note:r.note,tags:r.tags||[],exercises:r.exercises.map(o=>{const i=w.exerciseMap.get(o.exerciseId)||null;return{exerciseId:o.exerciseId,exercise:i,name:i?.name||o.exerciseId,reps:o.reps??i?.recommendations?.reps,sets:o.sets??i?.recommendations?.sets,repUnits:o.repUnits??i?.recommendations?.repUnits,note:o.note??i?.recommendations?.note}})}),n=(t.items||[]).map(r=>r.kind?a(r):s(r));return{...t,resolvedItems:n}}function ze(){return["localhost","127.0.0.1"].includes(window.location.hostname)}function Re(e){e.innerHTML=`
  ...
  `}function W(e){e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/programs"))}function ft(e,t){e.innerHTML=V(`
    <main class="flex-1 px-6 pt-12 pb-24">
      <div class="card p-6">
...
`),t}async function ns(e,t,s,a,n){const r=ne();if(!r)throw new Error("No API key configured");const i=[{role:"system",content:as(a)+rs(s)},...t.slice(-20),{role:"user",content:e}];let d=await fe(r,i),l=d.choices[0].message,u=0;for(;l.tool_calls&&u<5;){u++;const c=[];for(const p of l.tool_calls){const x=JSON.parse(p.function.arguments),g=await ss(p.function.name,x,s);c.push({role:"tool",tool_call_id:p.id,content:JSON.stringify(g)}),n?.({type:"tool",name:p.function.name,args:x,result:g})}i.push(l),i.push(...c),d=await fe(r,i),l=d.choices[0].message}return l.content||""}async function fe(e,t){const s=await fetch(Zt,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:Qt,messages:t,tools:ts,tool_choice:"auto",temperature:.3})});if(!s.ok){const a=await s.text();throw new Error(`OpenAI API error (${s.status}): ${a}`)}return s.json()}function os(e){const t={id:e.meta.id||"untitled",title:e.meta.title||"Untitled Program"};return e.meta.requirements&&(t.requirements=e.meta.requirements),t.items=e.items,{program:t,newExercises:e.newExercises}}async function is(e){const t=es(),s=[];let a=!1;const{exercises:n}=await A(),r=n.map(f=>({id:f.id,name:f.name}));e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/studio"));const o=e.querySelector('[data-region="messages"]'),i=e.querySelector('[data-input="message"]'),d=e.querySelector('[data-action="send"]'),l=e.querySelector('[data-action="export"]'),u=e.querySelector('[data-region="settings-modal"]'),c=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="settings"]')?.addEventListener("click",()=>{e.querySelector('[data-input="api-key"]').value=ne(),u.classList.remove("hidden")}),e.querySelector('[data-action="close-settings"]')?.addEventListener("click",()=>u.classList.add("hidden")),e.querySelector('[data-action="save-settings"]')?.addEventListener("click",()=>{const f=e.querySelector('[data-input="api-key"]').value.trim();Xt(f),u.classList.add("hidden")}),u?.addEventListener("click",f=>{f.target===u&&u.classList.add("hidden")}),l?.addEventListener("click",()=>L()),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>c.classList.add("hidden")),c?.addEventListener("click",f=>{f.target===c&&c.classList.add("hidden")}),xe()||u.classList.remove("hidden");async function p(){const f=i.value.trim();if(!f||a)return;if(!xe()){u.classList.remove("hidden");return}i.value="",a=!0,d.disabled=!0,x("user",f),s.push({role:"user",content:f});const k=x("assistant","...");k.dataset.loading="true";try{const $=await ns(f,s,t,r,h=>{h.type==="tool"&&g(h)});k.remove(),x("assistant",$),s.push({role:"assistant",content:$}),y()}catch($){k.remove(),x("error",$.message)}finally{a=!1,d.disabled=!1,i.focus()}}d?.addEventListener("click",p),i?.addEventListener("keydown",f=>{f.key==="Enter"&&!f.shiftKey&&(f.preventDefault(),p())});function x(f,k){const $=o.querySelector(".text-center.py-8");$&&f!=="error"&&$.remove();const h=document.createElement("div");h.className=f==="user"?"flex justify-end":"flex justify-start";const M=document.createElement("div");return f==="user"?M.className="bg-brand-500/20 text-slate-100 rounded-2xl rounded-br-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":f==="error"?M.className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":M.className="bg-slate-800/60 text-slate-200 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed",M.textContent=k,h.appendChild(M),o.appendChild(h),o.scrollTop=o.scrollHeight,h}function g(f){const k=document.createElement("div");k.className="flex justify-start";const $={search_exercises:`🔍 Searching: "${f.args.query}"`,add_exercise:`✓ Added: ${f.args.exerciseId}`,create_exercise:`✓ Created: ${f.args.name}`,remove_exercise:`✗ Removed item at position ${f.args.index}`,group_exercises:`⚡ Grouped as ${f.args.kind}`,set_metadata:`📝 Updated: ${f.args.title||f.args.requirements||"metadata"}`,update_exercise:`✏️ Updated item at position ${f.args.index}`}[f.name]||`🔧 ${f.name}`;k.innerHTML=`<span class="text-[11px] text-slate-500 italic px-2 py-1">${B($)}</span>`,o.appendChild(k),o.scrollTop=o.scrollHeight}function y(){const f=e.querySelector('[data-region="program-preview"]'),k=e.querySelector('[data-region="program-items"]'),$=e.querySelector('[data-region="item-count"]');if(t.items.length===0){f.classList.add("hidden"),l.classList.add("hidden");return}f.classList.remove("hidden"),l.classList.remove("hidden"),$.textContent=`${t.items.length} item${t.items.length!==1?"s":""}`,k.innerHTML=t.items.map((h,M)=>{if(h.kind)return`<div class="text-xs text-slate-400 pl-2 border-l-2 border-brand-500"><span class="text-brand-300 font-medium">${h.kind}</span>: ${h.exercises.map(je=>je.exerciseId).join(" + ")}</div>`;const T=h.tags?.length?`<span class="text-brand-300">[${h.tags.join(", ")}]</span> `:"";return`<div class="text-xs text-slate-300">${M+1}. ${T}${B(h.exerciseId)} — ${h.reps||"?"} ${h.repUnits||"reps"} × ${h.sets||"?"}</div>`}).join("")}function L(){const{program:f,newExercises:k}=os(t),$=e.querySelector('[data-region="export-content"]');let h="";if(k.length>0){const T=JSON.stringify(k,null,2);h+=`
      ...
      `}const M=JSON.stringify(f,null,2);h+=`
      <div class="space-y-2">
        <div class="flex items-center justify-between">
    ...
    `,$.innerHTML=h,$.querySelectorAll("[data-copy]").forEach(T=>{T.addEventListener("click",()=>{navigator.clipboard?.writeText(T.dataset.copy).then(()=>{T.textContent="✓ Copied",setTimeout(()=>{T.textContent="Copy"},2e3)})})}),c.classList.remove("hidden")}}function B(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function ls(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back()),e.querySelectorAll("[data-program-id]").forEach(o=>{o.addEventListener("click",()=>v(`/program/${o.dataset.programId}`))}),a.length>0){const o=e.querySelector('[data-region="demos"]');H(o,a)}}function ps(e,t){e.innerHTML=`
