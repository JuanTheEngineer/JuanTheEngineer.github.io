(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();const fe=[];let se=null;function j(e,t){const s=[],a=new RegExp("^"+e.replace(/:([^/]+)/g,(n,r)=>(s.push(r),"([^/]+)"))+"$");fe.push({pattern:e,regex:a,keys:s,handler:t})}function je(e){se=e}function w(e){window.location.hash=e.startsWith("#")?e:`#${e}`}function oe(){const e=window.location.hash.slice(1)||"/";for(const t of fe){const s=e.match(t.regex);if(s){const a={};t.keys.forEach((n,r)=>{a[n]=decodeURIComponent(s[r+1])}),t.handler(a);return}}se&&se(e)}function Te(){window.addEventListener("hashchange",oe),window.location.hash?oe():window.location.hash="#/"}const ge="action-app:progress",be="action-app:recent-programs",Pe=5;function ae(){try{return JSON.parse(localStorage.getItem(ge)||"{}")}catch{return{}}}function he(e){try{localStorage.setItem(ge,JSON.stringify(e))}catch{}}function ve(e){const t=ae();return new Set(t[e]||[])}function Ie(e,t){const s=ae(),a=new Set(s[e]||[]);return a.has(t)?a.delete(t):a.add(t),s[e]=Array.from(a),he(s),a}function Ae(e){const t=ae();delete t[e],he(t)}function ye(){try{const e=JSON.parse(localStorage.getItem(be)||"[]");return Array.isArray(e)?e:[]}catch{return[]}}function Ne(e){if(e)try{const t=ye().filter(s=>s.id!==e);t.unshift({id:e,visitedAt:Date.now()}),localStorage.setItem(be,JSON.stringify(t.slice(0,Pe)))}catch{}}const k={workouts:null,exercises:null,plans:null,exerciseMap:null};async function z(){if(k.workouts)return k.workouts;const e=await fetch("./workouts.json");if(!e.ok)throw new Error(`Failed to load workouts.json: ${e.status}`);return k.workouts=await e.json(),k.workouts}async function N(){if(k.exercises)return k.exercises;const e=await fetch("./exercises.json");if(!e.ok)throw new Error(`Failed to load exercises.json: ${e.status}`);return k.exercises=await e.json(),k.exerciseMap=new Map(k.exercises.exercises.map(t=>[t.id,t])),k.exercises}async function we(){if(k.plans)return k.plans;const e=await fetch("./plans.json");if(!e.ok)throw new Error(`Failed to load plans.json: ${e.status}`);return k.plans=await e.json(),k.plans}async function Be(e){return(await z()).programs.find(s=>s.id===e)||null}async function Ue(e){return await N(),k.exerciseMap?.get(e)||null}async function He(e){const[t]=await Promise.all([Be(e),N()]);if(!t)return null;const s=r=>{const o=k.exerciseMap.get(r.exerciseId)||null;return{kind:"single",exerciseId:r.exerciseId,exercise:o,name:o?.name||r.exerciseId,reps:r.reps??o?.recommendations?.reps,sets:r.sets??o?.recommendations?.sets,repUnits:r.repUnits??o?.recommendations?.repUnits,note:r.note??o?.recommendations?.note,tags:r.tags||[]}},a=r=>({kind:r.kind,note:r.note,tags:r.tags||[],exercises:r.exercises.map(o=>{const i=k.exerciseMap.get(o.exerciseId)||null;return{exerciseId:o.exerciseId,exercise:i,name:i?.name||o.exerciseId,reps:o.reps??i?.recommendations?.reps,sets:o.sets??i?.recommendations?.sets,repUnits:o.repUnits??i?.recommendations?.repUnits,note:o.note??i?.recommendations?.note}})}),n=(t.items||[]).map(r=>r.kind?a(r):s(r));return{...t,resolvedItems:n}}function _e(){return["localhost","127.0.0.1"].includes(window.location.hostname)}function ze(e){e.innerHTML=`
  ...
  `,e.querySelector('[data-action="create"]')?.addEventListener("click",()=>w("/studio")),e.querySelector('[data-action="search"]')?.addEventListener("click",()=>w("/search")),e.querySelector('[data-action="programs"]')?.addEventListener("click",()=>w("/programs")),e.querySelector('[data-action="exercises"]')?.addEventListener("click",()=>w("/exercises")),Re(e).catch(t=>console.warn("[recent] skipped",t))}async function Re(e){const t=ye();if(t.length===0)return;const s=e.querySelector('[data-region="recent"]');if(!s)return;const{programs:a}=await z(),n=new Map(a.map(o=>[o.id,o])),r=t.map(o=>({...o,program:n.get(o.id)})).filter(o=>o.program).slice(0,3);r.length!==0&&(s.classList.remove("hidden"),s.innerHTML=`
    ...
    <ul class="space-y-2">
      ${r.map(o=>Oe(o.program)).join("")}
    </ul>
  `,s.querySelectorAll("[data-program-id]").forEach(o=>{o.addEventListener("click",()=>w(`/program/${o.dataset.programId}`))}),s.querySelector('[data-action="all-recent"]')?.addEventListener("click",()=>w("/programs")))}function Oe(e){const t=e.items?.length||e.exercises?.length||0,s=ve(e.id).size,a=t>0?Math.round(s/t*100):0,n=s===0?"Not started":s>=t?"Complete":`${s} of ${t} done`;return`
    <li>
      <button
      ...
      >
        <div class="flex items-center gap-3">
          <div class="flex-1 min-w-0">
            <h3 class="font-semibold tracking-tight truncate">${De(e.title)}</h3>
  ...
  `}function De(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function Fe(e){e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>w("/"));try{const[t,s]=await Promise.all([z(),we()]);Ge(e,t.programs,s.plans)}catch(t){Ye(e,t)}}function Ge(e,t,s){const a=new Map(t.map(r=>[r.id,r])),n=[];for(const r of s)for(const o of r.subPlans||[]){const i=(o.programs||[]).map(d=>a.get(d)).filter(Boolean);i.length!==0&&n.push({category:r.name,title:o.name,description:o.description,programs:i})}e.innerHTML=`
  ...
  `}function ke(e){if(!e)return null;const t=[/youtube\.com\/watch\?v=([^&]+)/,/youtube\.com\/shorts\/([^?&/]+)/,/youtube\.com\/embed\/([^?&/]+)/,/youtu\.be\/([^?&/]+)/];for(const s of t){const a=e.match(s);if(a)return a[1]}return null}function We(e,t="hqdefault"){const s=ke(e);return s?`https://i.ytimg.com/vi/${s}/${t}.jpg`:null}function Ke(e,t={}){const s=ke(e);if(!s)return null;const a=new URLSearchParams({autoplay:"1",rel:"0",modestbranding:"1",playsinline:"1"});return t.startTime&&a.set("start",String(Math.floor(t.startTime))),t.endTime&&a.set("end",String(Math.floor(t.endTime))),`https://www.youtube.com/embed/${s}?${a.toString()}`}function $e(e,t="w_800,q_auto,f_auto"){return!e||!e.includes("cloudinary.com")?e:e.replace("/upload/",`/upload/${t}/`)}function Ve(e){if(!e||!e.type)return"unknown";if(["youtube","tiktok","vimeo"].includes(e.type))return"embed";const t=e.mediaType==="video"||["mp4","webm","mov"].includes(e.format);return e.format==="gif"?"image":t?"video":"image"}function Ze(e,t,s={}){if(!t){e.innerHTML='<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">No media</div>';return}const a=Ve(t),n=s.className||"w-full max-h-[60vh] object-contain rounded-2xl bg-slate-800";switch(e.classList.add("animate-fade-in"),a){case"image":Qe(e,t,n,s.onError);break;case"video":Xe(e,t,n,s.autoplay,s.onError);break;case"embed":et(e,t,n,s.onEmbedPlay);break;default:e.innerHTML=`<div class="${n} flex items-center justify-center text-slate-500 text-sm">Unsupported media type</div>`}}function Qe(e,t,s,a){const n=t.type==="cloudinary"?$e(t.url,"w_800,q_auto,f_auto"):t.url;e.innerHTML=`
    <img
      src="${n}"
  ...
  `;const d=e.querySelector('[data-action="play-embed"]');d&&d.addEventListener("click",()=>{const l=t.type==="youtube"?Ke(t.url,{startTime:t.startTime,endTime:t.endTime}):t.url;e.innerHTML=`
      ...
      `,a?.()},{once:!0})}function _(e,t,s={}){let a=(t||[]).filter(Boolean);if(a.length===0){e.innerHTML='<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">No demos available</div>';return}a=st(a);let n=tt(s.startIndex??0,a.length);e.innerHTML=`
    <div class="relative">
      <div
        data-region="track"
  ...
  `;const r=e.querySelector('[data-region="track"]'),o=e.querySelector('[data-region="dots"]'),i=e.querySelector('[data-region="caption"]');r.style.scrollbarWidth="none",a.forEach((x,g)=>{const v=document.createElement("div");v.className="shrink-0 w-full snap-center",v.dataset.slideIndex=String(g),r.appendChild(v)});const d=new Set,l=new Set,p=x=>{if(d.has(x))return;const g=r.children[x];if(!g)return;const v=a[x];Ze(g,v,{onError:()=>u(x),onEmbedPlay:()=>l.add(x)}),d.add(x)},c=x=>{!l.has(x)||!r.children[x]||(l.delete(x),d.delete(x),p(x))},u=x=>{const g=r.children[x];g&&(g.innerHTML=`<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">Couldn't load demo</div>`)},b=()=>{o&&(o.innerHTML=a.map((x,g)=>`
      <button
        data-dot-index="${g}"
    ...
    `).join(""),o.querySelectorAll("[data-dot-index]").forEach(x=>{x.addEventListener("click",()=>S(Number(x.dataset.dotIndex)))}))},h=()=>{i&&(i.textContent=at(a[n]))};function S(x){const g=r.children[x];g&&r.scrollTo({left:g.offsetLeft-r.offsetLeft,behavior:"smooth"})}let $=!1;const f=()=>{$||($=!0,requestAnimationFrame(()=>{$=!1;const x=r.children[0]?.offsetWidth||1,g=Math.round(r.scrollLeft/x);if(g!==n&&g>=0&&g<a.length){const v=n;n=g,c(v),b(),h(),p(n),n+1<a.length&&p(n+1),n-1>=0&&p(n-1)}}))};r.addEventListener("scroll",f,{passive:!0}),p(n),n+1<a.length&&p(n+1),n-1>=0&&p(n-1),requestAnimationFrame(()=>{const x=r.children[n];x&&(r.scrollLeft=x.offsetLeft-r.offsetLeft)}),b(),h()}function tt(e,t){return Math.max(0,Math.min(t-1,e))}function st(e){const t={cloudinary:0,youtube:1,vimeo:2,tiktok:2,url:3,local:4};return[...e].sort((s,a)=>s.isPrimary&&!a.isPrimary?-1:a.isPrimary&&!s.isPrimary?1:(t[s.type]??99)-(t[a.type]??99))}function at(e){const t={cloudinary:e.format==="mp4"?"Video":"Demo",youtube:"YouTube",tiktok:"TikTok",vimeo:"Vimeo",local:"Demo",url:"External"}[e.type]||e.type;return e.notes?`${t} · ${e.notes}`:t}function rt(e,t){const s=document.createElement("article");s.className=`card overflow-hidden transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=e.exercise?.demos||[],r=`${t.index+1}. ${e.name}`;if(s.innerHTML=`
  ...
  `}function I(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const ie=["Nice work!","Killer moves!","Awesome job!","Crushed it!","You did it!","Beast mode!","On fire!","Way to go!","Strengthened and Conditioned!"];let le=0;function pt(){const e=Date.now();if(e-le<5e3)return;le=e;const t=ie[Math.floor(Math.random()*ie.length)],s=document.createElement("div");s.className="celebration-flash";const a=document.createElement("div");a.className="celebration-text",a.textContent=t,document.body.appendChild(s),document.body.appendChild(a),setTimeout(()=>s.remove(),700),setTimeout(()=>a.remove(),3100)}async function ut(e,t){e.innerHTML=W(`
    <main class="flex-1 px-6 pb-24 flex items-center justify-center">
      <div class="w-8 h-8 border-2 border-brand-500 border-t-transparent rounded-full animate-spin"></div>
    </main>
  `),K(e);try{const s=await He(t);if(!s){xt(e,t);return}Ne(s.id),mt(e,s)}catch(s){ft(e,s)}}function mt(e,t){const s=ve(t.id),a=t.resolvedItems.length;let n=-1;e.innerHTML=W(`
  ...
  `,t.title),K(e);const r=e.querySelector('[data-region="items"]'),o=e.querySelector('[data-region="actions"]'),i=()=>{r.innerHTML="";let p=null;t.resolvedItems.forEach((c,u)=>{const b=c.tags||[];let h="main";if(b.includes("warmup")?h="warmup":b.includes("stretch")&&(h="stretch"),h!==p){p=h;const x=document.createElement("li");x.className="pt-4 pb-1 first:pt-0";const g={warmup:"WARM-UP",main:"MAIN WORK",stretch:"COOL-DOWN"}[h];x.innerHTML=`<p class="eyebrow border-b border-slate-800 pb-2">${g}</p>`,r.appendChild(x)}const S=document.createElement("li"),$={index:u,isExpanded:u===n,isCompleted:s.has(u),onToggle:x=>{n=n===x?-1:x,l(),n===x&&requestAnimationFrame(()=>{const g=r.querySelector(`[data-item-index="${x}"]`);if(g){const v=window.scrollY+g.getBoundingClientRect().top-130;window.scrollTo({top:Math.max(0,v),behavior:"smooth"})}})},onComplete:x=>{const g=s.size===a,v=Ie(t.id,x);s.clear(),v.forEach(q=>s.add(q)),v.has(x)&&n===x&&(n=-1),l(),!g&&s.size===a&&setTimeout(pt,250)}},f=c.kind==="single"?rt(c,$):dt(c,$);S.appendChild(f),r.appendChild(S)})},d=()=>{const p=e.querySelector('[data-region="progress-bar"]');p&&(p.style.width=`${s.size/a*100}%`);const c=e.querySelector('[data-region="completed-count"]');c&&(c.textContent=String(s.size)),o.classList.toggle("hidden",s.size===0)},l=()=>{i(),d()};l(),e.querySelector('[data-action="reset"]')?.addEventListener("click",()=>{confirm("Reset progress for this program?")&&(Ae(t.id),s.clear(),l())}),e.querySelector('[data-action="share"]')?.addEventListener("click",()=>{const p=window.location.href;navigator.share?navigator.share({title:t.title,text:`Check out: ${t.title}`,url:p}).catch(()=>{}):navigator.clipboard?.writeText(p).then(()=>alert("Link copied!")).catch(()=>prompt("Copy:",p))})}function W(e,t="Program"){return`
    <div class="flex-1 flex flex-col">
  ...
  `}function K(e){e.querySelector('[data-action="back"]')?.addEventListener("click",()=>w("/programs"))}function xt(e,t){e.innerHTML=W(`
    <main class="flex-1 px-6 pt-12 pb-24">
      <div class="card p-6">
  ...
  `),K(e)}function ft(e,t){e.innerHTML=W(`
    <main class="flex-1 px-6 pt-12 pb-24">
      <div class="card p-6">
  ...
  `),K(e)}function O(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function gt(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
Do you still want to create "${d}"?`))return;const u=s.querySelector('[data-exfield="reps"]').value,b=s.querySelector('[data-exfield="sets"]').value,h=s.querySelector('[data-exfield="repUnits"]').value,S=s.querySelector('[data-exfield="note"]').value,$={id:l,name:d,demos:ee.filter(f=>f.url),recommendations:{}};u&&($.recommendations.reps=u),b&&($.recommendations.sets=b),h&&h!=="reps"&&($.recommendations.repUnits=h),S&&($.recommendations.note=S),m.newExercises.push($),ht($),m.items.push({type:"single",exerciseId:l,exerciseName:d,exerciseNote:S||"",reps:u||"",sets:b||"",repUnits:h||"reps",note:"",tags:[]}),s.classList.add("hidden"),L(e)},{once:!0});const i=()=>s.classList.add("hidden");s.querySelector('[data-action="cancel-exercise"]')?.addEventListener("click",i,{once:!0}),s.querySelector('[data-action="close-exercise"]')?.addEventListener("click",i,{once:!0}),s.classList.remove("hidden")}function zt(e){const t=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="export"]')?.addEventListener("click",()=>{Dt(e)}),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>{t?.classList.add("hidden")}),t?.addEventListener("click",s=>{s.target===t&&t.classList.add("hidden")}),e.querySelector('[data-action="preview"]')?.addEventListener("click",()=>{Rt(e)})}function Rt(e){const t=Le(),s=t.items||[],a=`
  ...
  `,Gt(e),Jt(e),Wt(e),Kt(e)}function Gt(e){e.querySelector('[data-action="back"]')?.addEventListener("click",()=>w("/studio"))}function Jt(e){const t=e.querySelector('[data-input="edit-search"]'),s=e.querySelector('[data-region="edit-results"]');let a=null;t?.addEventListener("input",()=>{clearTimeout(a),a=setTimeout(async()=>{const n=t.value.trim();if(!n){s.classList.add("hidden");return}const r=await V(n,8);if(r.length===0){s.classList.add("hidden");return}s.classList.remove("hidden"),s.innerHTML=r.map(o=>`
...
`),t}async function rs(e,t,s,a,n){const r=ne();if(!r)throw new Error("No API key configured");const i=[{role:"system",content:ss(a)+as(s)},...t.slice(-20),{role:"user",content:e}];let d=await xe(r,i),l=d.choices[0].message,p=0;for(;l.tool_calls&&p<5;){p++;const c=[];for(const u of l.tool_calls){const b=JSON.parse(u.function.arguments),h=await ts(u.function.name,b,s);c.push({role:"tool",tool_call_id:u.id,content:JSON.stringify(h)}),n?.({type:"tool",name:u.function.name,args:b,result:h})}i.push(l),i.push(...c),d=await xe(r,i),l=d.choices[0].message}return l.content||""}async function xe(e,t){const s=await fetch(Vt,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:Zt,messages:t,tools:es,tool_choice:"auto",temperature:.3})});if(!s.ok){const a=await s.text();throw new Error(`OpenAI API error (${s.status}): ${a}`)}return s.json()}function ns(e){const t={id:e.meta.id||"untitled",title:e.meta.title||"Untitled Program"};return e.meta.requirements&&(t.requirements=e.meta.requirements),t.items=e.items,{program:t,newExercises:e.newExercises}}async function os(e){const t=Xt(),s=[];let a=!1;const{exercises:n}=await N(),r=n.map(f=>({id:f.id,name:f.name}));e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>w("/studio"));const o=e.querySelector('[data-region="messages"]'),i=e.querySelector('[data-input="message"]'),d=e.querySelector('[data-action="send"]'),l=e.querySelector('[data-action="export"]'),p=e.querySelector('[data-region="settings-modal"]'),c=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="settings"]')?.addEventListener("click",()=>{e.querySelector('[data-input="api-key"]').value=ne(),p.classList.remove("hidden")}),e.querySelector('[data-action="close-settings"]')?.addEventListener("click",()=>p.classList.add("hidden")),e.querySelector('[data-action="save-settings"]')?.addEventListener("click",()=>{const f=e.querySelector('[data-input="api-key"]').value.trim();Qt(f),p.classList.add("hidden")}),p?.addEventListener("click",f=>{f.target===p&&p.classList.add("hidden")}),l?.addEventListener("click",()=>$()),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>c.classList.add("hidden")),c?.addEventListener("click",f=>{f.target===c&&c.classList.add("hidden")}),me()||p.classList.remove("hidden");async function u(){const f=i.value.trim();if(!f||a)return;if(!me()){p.classList.remove("hidden");return}i.value="",a=!0,d.disabled=!0,b("user",f),s.push({role:"user",content:f});const x=b("assistant","...");x.dataset.loading="true";try{const g=await rs(f,s,t,r,v=>{v.type==="tool"&&h(v)});x.remove(),b("assistant",g),s.push({role:"assistant",content:g}),S()}catch(g){x.remove(),b("error",g.message)}finally{a=!1,d.disabled=!1,i.focus()}}d?.addEventListener("click",u),i?.addEventListener("keydown",f=>{f.key==="Enter"&&!f.shiftKey&&(f.preventDefault(),u())});function b(f,x){const g=o.querySelector(".text-center.py-8");g&&f!=="error"&&g.remove();const v=document.createElement("div");v.className=f==="user"?"flex justify-end":"flex justify-start";const q=document.createElement("div");return f==="user"?q.className="bg-brand-500/20 text-slate-100 rounded-2xl rounded-br-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":f==="error"?q.className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":q.className="bg-slate-800/60 text-slate-200 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed",q.textContent=x,v.appendChild(q),o.appendChild(v),o.scrollTop=o.scrollHeight,v}function h(f){const x=document.createElement("div");x.className="flex justify-start";const g={search_exercises:`🔍 Searching: "${f.args.query}"`,add_exercise:`✓ Added: ${f.args.exerciseId}`,create_exercise:`✓ Created: ${f.args.name}`,remove_exercise:`✗ Removed item at position ${f.args.index}`,group_exercises:`⚡ Grouped as ${f.args.kind}`,set_metadata:`📝 Updated: ${f.args.title||f.args.requirements||"metadata"}`,update_exercise:`✏️ Updated item at position ${f.args.index}`}[f.name]||`🔧 ${f.name}`;x.innerHTML=`<span class="text-[11px] text-slate-500 italic px-2 py-1">${B(g)}</span>`,o.appendChild(x),o.scrollTop=o.scrollHeight}function S(){const f=e.querySelector('[data-region="program-preview"]'),x=e.querySelector('[data-region="program-items"]'),g=e.querySelector('[data-region="item-count"]');if(t.items.length===0){f.classList.add("hidden"),l.classList.add("hidden");return}f.classList.remove("hidden"),l.classList.remove("hidden"),g.textContent=`${t.items.length} item${t.items.length!==1?"s":""}`,x.innerHTML=t.items.map((v,q)=>{if(v.kind)return`<div class="text-xs text-slate-400 pl-2 border-l-2 border-brand-500"><span class="text-brand-300 font-medium">${v.kind}</span>: ${v.exercises.map(Me=>Me.exerciseId).join(" + ")}</div>`;const T=v.tags?.length?`<span class="text-brand-300">[${v.tags.join(", ")}]</span> `:"";return`<div class="text-xs text-slate-300">${q+1}. ${T}${B(v.exerciseId)} — ${v.reps||"?"} ${v.repUnits||"reps"} × ${v.sets||"?"}</div>`}).join("")}function $(){const{program:f,newExercises:x}=ns(t),g=e.querySelector('[data-region="export-content"]');let v="";if(x.length>0){const T=JSON.stringify(x,null,2);v+=`
        <div class="space-y-2">
          <div class="flex items-center justify-between">
      ...
      `}const q=JSON.stringify(f,null,2);v+=`
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          <p class="text-xs text-slate-400">Program (append to workouts.json)</p>
          <button data-copy="${B(q)}" class="text-xs text-brand-400 hover:text-brand-300">Copy</button>
        </div>
        <pre class="bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs text-slate-300 overflow-x-auto max-h-[300px] overflow-y-auto font-mono">${B(q)}</pre>
      </div>
    `,g.innerHTML=v,g.querySelectorAll("[data-copy]").forEach(T=>{T.addEventListener("click",()=>{navigator.clipboard?.writeText(T.dataset.copy).then(()=>{T.textContent="✓ Copied",setTimeout(()=>{T.textContent="Copy"},2e3)})})}),c.classList.remove("hidden")}}function B(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function is(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>w("/"));const{exercises:t}=await N(),s=e.querySelector('[data-region="count"]');s.textContent=`${t.length} total`;let a=null;const n=l=>{const p=e.querySelector('[data-region="list"]'),c=e.querySelector('[data-region="empty"]');if(l.length===0){p.classList.add("hidden"),c.classList.remove("hidden");return}if(p.classList.remove("hidden"),c.classList.add("hidden"),p.innerHTML=l.map(u=>`
    ...
    `).join(""),p.querySelectorAll('[data-action="expand"]').forEach(u=>{u.addEventListener("click",()=>{if(a=a===u.dataset.id?null:u.dataset.id,n(l),a){const b=p.querySelector(`[data-demo-slot="${a}"]`),h=l.find(S=>S.id===a);b&&h&&h.demos.length>0&&_(b,h.demos)}})}),a){const u=p.querySelector(`[data-demo-slot="${a}"]`),b=l.find(h=>h.id===a);u&&b&&b.demos.length>0&&_(u,b.demos)}};function r(l){return`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back());const s=await Ue(t);if(!s){cs(e,t);return}const{programs:a}=await z(),n=ps(a,t);ds(e,s,n)}function ds(e,t,s){const a=t.demos||[],n=t.recommendations||{},r=t.aliases||[];if(e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-4 flex items-center gap-3">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back()),e.querySelectorAll("[data-program-id]").forEach(o=>{o.addEventListener("click",()=>w(`/program/${o.dataset.programId}`))}),a.length>0){const o=e.querySelector('[data-region="demos"]');_(o,a)}}function cs(e,t){e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>w("/"));const[{programs:t},{plans:s}]=await Promise.all([z(),we()]),a=t.map(c=>({id:c.id,title:c.title,requirements:c.requirements||"",itemCount:c.items?.length||c.exercises?.length||0,tokens:J(c.title).concat(J(c.requirements||"")).concat(J(c.id.replace(/[-_]/g," "))),program:c})),n=new Map;for(const c of s)for(const u of c.subPlans||[])for(const b of u.programs||[])n.set(b,`${c.name} · ${u.name}`);const r=e.querySelector('[data-region="results"]'),o=e.querySelector('[data-region="empty"]'),i=e.querySelector('[data-region="initial"]');function d(c){if(c===null){r.classList.add("hidden"),o.classList.add("hidden"),i.classList.remove("hidden");return}if(i.classList.add("hidden"),c.length===0){r.classList.add("hidden"),o.classList.remove("hidden");return}o.classList.add("hidden"),r.classList.remove("hidden"),r.innerHTML=c.map(u=>ms(u,n.get(u.id))).join(""),r.querySelectorAll("[data-program-id]").forEach(u=>{u.addEventListener("click",()=>w(`/program/${u.dataset.programId}`))})}const l=e.querySelector('[data-input="search"]');let p=null;l?.addEventListener("input",()=>{clearTimeout(p),p=setTimeout(()=>{const c=l.value.trim();if(!c){d(null);return}const u=J(c),b=a.map(h=>({...h,score:xs(h.tokens,u)})).filter(h=>h.score>0).sort((h,S)=>S.score-h.score);d(b)},150)}),d(null)}function ms(e,t){return`
    <button
      data-program-id="${e.id}"
      class="w-full card p-4 text-left active:scale-[0.98] transition-transform"
    >
  ...
  `}function J(e){return e?e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>0):[]}function xs(e,t){let s=0;for(const a of t){let n=0;for(const r of e)r===a?n=Math.max(n,10):r.startsWith(a)?n=Math.max(n,7):r.includes(a)&&(n=Math.max(n,4));if(n===0)return 0;s+=n}return s}function te(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const M=document.getElementById("app");j("/",()=>ze(M));j("/programs",()=>Fe(M));j("/program/:id",({id:e})=>ut(M,e));j("/exercises",()=>is(M));j("/exercise/:id",({id:e})=>ls(M,e));j("/search",()=>us(M));const fs=["localhost","127.0.0.1"].includes(window.location.hostname);fs&&(j("/studio",()=>gt(M)),j("/studio/program",()=>Tt(M)),j("/studio/exercise",()=>Ft(M)),j("/studio/ai",()=>os(M)));je(e=>{M.innerHTML=`
    <div class="flex-1 flex flex-col items-center justify-center min-h-screen px-6 text-center">
      <p class="text-6xl mb-4">🤔</p>
