(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();const ge=[];let se=null;function j(e,t){const s=[],a=new RegExp("^"+e.replace(/:([^/]+)/g,(n,r)=>(s.push(r),"([^/]+)"))+"$");ge.push({pattern:e,regex:a,keys:s,handler:t})}function Te(e){se=e}function v(e){window.location.hash=e.startsWith("#")?e:`#${e}`}function oe(){const e=window.location.hash.slice(1)||"/";for(const t of ge){const s=e.match(t.regex);if(s){const a={};t.keys.forEach((n,r)=>{a[n]=decodeURIComponent(s[r+1])}),t.handler(a);return}}se&&se(e)}function Pe(){window.addEventListener("hashchange",oe),window.location.hash?oe():window.location.hash="#/"}const be="action-app:progress",ve="action-app:recent-programs",Ie=5;function ae(){try{return JSON.parse(localStorage.getItem(be)||"{}")}catch{return{}}}function he(e){try{localStorage.setItem(be,JSON.stringify(e))}catch{}}function ye(e){const t=ae();return new Set(t[e]||[])}function Ae(e,t){const s=ae(),a=new Set(s[e]||[]);return a.has(t)?a.delete(t):a.add(t),s[e]=Array.from(a),he(s),a}function Be(e){const t=ae();delete t[e],he(t)}function we(){try{const e=JSON.parse(localStorage.getItem(ve)||"[]");return Array.isArray(e)?e:[]}catch{return[]}}function Ne(e){if(e)try{const t=we().filter(s=>s.id!==e);t.unshift({id:e,visitedAt:Date.now()}),localStorage.setItem(ve,JSON.stringify(t.slice(0,Ie)))}catch{}}const w={workouts:null,exercises:null,plans:null,exerciseMap:null};async function _(){if(w.workouts)return w.workouts;const e=await fetch("./workouts.json");if(!e.ok)throw new Error(`Failed to load workouts.json: ${e.status}`);return w.workouts=await e.json(),w.workouts}async function A(){if(w.exercises)return w.exercises;const e=await fetch("./exercises.json");if(!e.ok)throw new Error(`Failed to load exercises.json: ${e.status}`);return w.exercises=await e.json(),w.exerciseMap=new Map(w.exercises.exercises.map(t=>[t.id,t])),w.exercises}async function ke(){if(w.plans)return w.plans;const e=await fetch("./plans.json");if(!e.ok)throw new Error(`Failed to load plans.json: ${e.status}`);return w.plans=await e.json(),w.plans}async function Ue(e){return(await _()).programs.find(s=>s.id===e)||null}async function He(e){return await A(),w.exerciseMap?.get(e)||null}async function _e(e){const[t]=await Promise.all([Ue(e),A()]);if(!t)return null;const s=r=>{const o=w.exerciseMap.get(r.exerciseId)||null;return{kind:"single",exerciseId:r.exerciseId,exercise:o,name:o?.name||r.exerciseId,reps:r.reps??o?.recommendations?.reps,sets:r.sets??o?.recommendations?.sets,repUnits:r.repUnits??o?.recommendations?.repUnits,note:r.note??o?.recommendations?.note,tags:r.tags||[]}},a=r=>({kind:r.kind,note:r.note,tags:r.tags||[],exercises:r.exercises.map(o=>{const i=w.exerciseMap.get(o.exerciseId)||null;return{exerciseId:o.exerciseId,exercise:i,name:i?.name||o.exerciseId,reps:o.reps??i?.recommendations?.reps,sets:o.sets??i?.recommendations?.sets,repUnits:o.repUnits??i?.recommendations?.repUnits,note:o.note??i?.recommendations?.note}})}),n=(t.items||[]).map(r=>r.kind?a(r):s(r));return{...t,resolvedItems:n}}function ze(){return["localhost","127.0.0.1"].includes(window.location.hostname)}function Re(e){e.innerHTML=`
  ...
  `}function Fe(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function Je(e){e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/")),e.querySelectorAll("[data-program-id]").forEach(r=>{r.addEventListener("click",()=>{v(`/program/${r.dataset.programId}`)})})}function Ye(e){const t=e.items?.length||e.exercises?.length||0;return`
    <li>
      <button
        data-program-id="${e.id}"
        class="w-full card p-4 text-left active:scale-[0.98] transition-transform"
  ...
  `}function $e(e){if(!e)return null;const t=[/youtube\.com\/watch\?v=([^&]+)/,/youtube\.com\/shorts\/([^?&/]+)/,/youtube\.com\/embed\/([^?&/]+)/,/youtu\.be\/([^?&/]+)/];for(const s of t){const a=e.match(s);if(a)return a[1]}return null}function We(e,t="hqdefault"){const s=$e(e);return s?`https://i.ytimg.com/vi/${s}/${t}.jpg`:null}function Ke(e,t={}){const s=$e(e);if(!s)return null;const a=new URLSearchParams({autoplay:"1",rel:"0",modestbranding:"1",playsinline:"1"});return t.startTime&&a.set("start",String(Math.floor(t.startTime))),t.endTime&&a.set("end",String(Math.floor(t.endTime))),`https://www.youtube.com/embed/${s}?${a.toString()}`}function Se(e,t="w_800,q_auto,f_auto"){return!e||!e.includes("cloudinary.com")?e:e.replace("/upload/",`/upload/${t}/`)}function Ze(e){if(!e||!e.type)return"unknown";if(["youtube","tiktok","vimeo"].includes(e.type))return"embed";const t=e.mediaType==="video"||["mp4","webm","mov"].includes(e.format);return e.format==="gif"?"image":t?"video":"image"}function ie(e,t,s={}){if(!t){e.innerHTML='<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">No media</div>';return}const a=Ze(t),n=s.className||"w-full max-h-[60vh] object-contain rounded-2xl bg-slate-800";switch(e.classList.add("animate-fade-in"),a){case"image":Qe(e,t,n,s.onError);break;case"video":Xe(e,t,n,s.autoplay,s.onError);break;case"embed":et(e,t,n,s.onEmbedPlay);break;default:e.innerHTML=`<div class="${n} flex items-center justify-center text-slate-500 text-sm">Unsupported media type</div>`}}function Qe(e,t,s,a){const n=t.type==="cloudinary"?Se(t.url,"w_800,q_auto,f_auto"):t.url;e.innerHTML=`
    <img
      ...
      decoding="async"
    />
  `,a&&e.querySelector("img")?.addEventListener("error",()=>a(),{once:!0})}function Xe(e,t,s,a=!0,n){const r=t.type==="cloudinary"?Se(t.url,"w_800,q_auto,f_auto"):t.url,o=t.startTime?`#t=${t.startTime}`:"";e.innerHTML=`
    <video
      src="${r}${o}"
  ...
  `;const d=e.querySelector('[data-action="play-embed"]');d&&d.addEventListener("click",()=>{const l=t.type==="youtube"?Ke(t.url,{startTime:t.startTime,endTime:t.endTime}):t.url;e.innerHTML=`
      ...
      `,a?.()},{once:!0})}function H(e,t){const s=rt((t||[]).filter(Boolean));if(s.length===0){e.innerHTML='<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">No demos available</div>';return}const a=new Set;let n=!1;const r=new Set(["youtube","tiktok","vimeo"]);s.forEach((i,d)=>{r.has(i.type)&&a.add(d)}),a.size>0?o():e.innerHTML=`
    ...
    `,s.forEach((i,d)=>{r.has(i.type)||tt(i,l=>{l&&(a.add(d),o())})}),setTimeout(()=>{a.size===0&&(s.forEach((i,d)=>a.add(d)),o())},6e3);function o(){const i=s.filter((u,c)=>a.has(c));if(i.length===0)return;const d=s.findIndex((u,c)=>a.has(c)),l=n?-1:i.indexOf(s[d]);st(e,i,l>=0?l:0,u=>{n=n||u})}}function tt(e,t){const s=e.url;if(!s){t(!1);return}if(e.mediaType==="video"||["mp4","webm","mov"].includes(e.format)){const n=document.createElement("video");n.preload="metadata",n.src=s,n.addEventListener("loadeddata",()=>t(!0),{once:!0}),n.addEventListener("error",()=>t(!1),{once:!0})}else{const n=new Image;if(n.src=s,n.complete&&n.naturalWidth>0){t(!0);return}n.addEventListener("load",()=>t(!0),{once:!0}),n.addEventListener("error",()=>t(!1),{once:!0})}}function st(e,t,s,a){let n=at(s,t.length);e.innerHTML=`
    <div class="relative">
      <div
        data-region="track"
  ...
  `;const r=e.querySelector('[data-region="track"]'),o=e.querySelector('[data-region="dots"]'),i=e.querySelector('[data-region="caption"]');r.style.scrollbarWidth="none";const d=new Set;t.forEach((p,f)=>{const g=document.createElement("div");g.className="shrink-0 w-full snap-center",ie(g,p,{onError:()=>{g.innerHTML=`<div class="aspect-video bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 text-sm">Couldn't load</div>`},onEmbedPlay:()=>d.add(f)}),r.appendChild(g)});const l=()=>{o&&(o.innerHTML=t.map((p,f)=>`
        ...
        </button>`).join(""),o.querySelectorAll("[data-dot-index]").forEach(p=>{p.addEventListener("click",()=>{a(!0);const f=Number(p.dataset.dotIndex),g=r.children[f];g&&r.scrollTo({left:g.offsetLeft-r.offsetLeft,behavior:"smooth"})})}))},u=()=>{i&&(i.textContent=nt(t[n]))};let c=!1;r.addEventListener("scroll",()=>{c||(c=!0,requestAnimationFrame(()=>{c=!1;const p=r.children[0]?.offsetWidth||1,f=Math.round(r.scrollLeft/p);if(f!==n&&f>=0&&f<t.length){if(a(!0),d.has(n)){const g=r.children[n];g&&(d.delete(n),ie(g,t[n],{onError:()=>{},onEmbedPlay:()=>d.add(n)}))}n=f,l(),u()}}))},{passive:!0}),requestAnimationFrame(()=>{const p=r.children[n];p&&(r.scrollLeft=p.offsetLeft-r.offsetLeft)}),l(),u()}function at(e,t){return Math.max(0,Math.min(t-1,e))}function rt(e){const t={cloudinary:0,youtube:1,vimeo:2,tiktok:2,url:3,local:4};return[...e].sort((s,a)=>s.isPrimary&&!a.isPrimary?-1:a.isPrimary&&!s.isPrimary?1:(t[s.type]??99)-(t[a.type]??99))}function nt(e){const t={cloudinary:e.format==="mp4"?"Video":"Demo",youtube:"YouTube",tiktok:"TikTok",vimeo:"Vimeo",local:"Demo",url:"External"}[e.type]||e.type;return e.notes?`${t} · ${e.notes}`:t}function ot(e,t){const s=document.createElement("article");s.className=`card overflow-hidden transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=e.exercise?.demos||[],r=`${t.index+1}. ${e.name}`;if(s.innerHTML=`
      ...
      <div class="px-4 pb-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-slate-800/50 rounded-xl p-3 text-center overflow-hidden">
            <p class="${(e.reps||"").length>5?"text-lg":"text-3xl"} font-extrabold text-brand-400 leading-none num tracking-tight">${N(e.reps||"—")}</p>
            <p class="label-meta mt-1.5">${N(e.repUnits||"reps")}</p>
          </div>
          <div class="bg-slate-800/50 rounded-xl p-3 text-center overflow-hidden">
            <p class="${(e.sets||"").length>5?"text-lg":"text-3xl"} font-extrabold text-brand-400 leading-none num tracking-tight">${N(e.sets||"—")}</p>
  ...
  `,t.isExpanded&&a.length>0){const o=s.querySelector("[data-media-slot]");o&&H(o,a)}return s.querySelector('[data-action="toggle"]')?.addEventListener("click",()=>{t.onToggle?.(t.index)}),s.querySelector('[data-action="complete"]')?.addEventListener("click",o=>{o.stopPropagation(),t.onComplete?.(t.index)}),s}function it(e){const t=e.reps||"—",s=e.sets||"—";return`${t} · ${s} sets`}function lt(e=[]){return e.length?`
  ...
  `:""}function N(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const dt={superset:"Super Set",compound:"Compound",circuit:"Circuit"};function ct(e,t){const s=document.createElement("article");s.className=`transition-all ${t.isCompleted?"opacity-60":""}`,s.dataset.itemIndex=String(t.index);const a=dt[e.kind]||e.kind,n=t.index+1,r=e.exercises.map((o,i)=>{const d=String.fromCharCode(97+i),l=`${n}${d}. ${z(o.name)}`,u=i===0,c=i===e.exercises.length-1;return`
      <div class="card ${u?"rounded-t-2xl":"rounded-t-none"} ${c?"rounded-b-2xl":"rounded-b-none"} overflow-hidden ${u?"":"border-t-0"}">
        ...
        <div class="flex items-stretch">
          ...
          >
            <div class="flex-1 min-w-0">
              ${u?`<div class="flex gap-1.5 mb-1.5"><span class="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md bg-brand-500/20 text-brand-300">${a}</span></div>`:""}
              <h3 class="font-semibold tracking-tight leading-tight ${t.isCompleted?"line-through text-slate-500":"text-slate-100"}">
                ${l}
              </h3>
              <p class="text-sm text-slate-400 mt-1 num truncate">
        ...
        <div data-region="member-content-${i}" class="${t.isExpanded?"":"hidden"}">
          <div class="px-4 pb-4 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="bg-slate-800/50 rounded-xl p-3 text-center overflow-hidden">
                <p class="${(o.reps||"").length>5?"text-lg":"text-3xl"} font-extrabold text-brand-400 leading-none num tracking-tight">${z(o.reps||"—")}</p>
                <p class="label-meta mt-1.5">${z(o.repUnits||"reps")}</p>
              </div>
              <div class="bg-slate-800/50 rounded-xl p-3 text-center overflow-hidden">
                <p class="${(o.sets||"").length>5?"text-lg":"text-3xl"} font-extrabold text-brand-400 leading-none num tracking-tight">${z(o.sets||"—")}</p>
                <p class="label-meta mt-1.5">sets</p>
              </div>
            </div>
            ${o.note?`
    ...
    `}).join("");return s.innerHTML=r,s.querySelectorAll('[data-action="toggle-member"]').forEach(o=>{o.addEventListener("click",()=>{t.onToggle?.(t.index)})}),s.querySelectorAll('[data-action="complete"]').forEach(o=>{o.addEventListener("click",i=>{i.stopPropagation(),t.onComplete?.(t.index)})}),t.isExpanded&&e.exercises.forEach((o,i)=>{const d=s.querySelector(`[data-member-media="${i}"]`),l=o.exercise?.demos||[];d&&l.length>0&&H(d,l)}),s}function z(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const le=["Nice work!","Killer moves!","Awesome job!","Crushed it!","You did it!","Beast mode!","On fire!","Way to go!","Strengthened and Conditioned!"];let de=0;function pt(){const e=Date.now();if(e-de<5e3)return;de=e;const t=le[Math.floor(Math.random()*le.length)],s=document.createElement("div");s.className="celebration-flash";const a=document.createElement("div");a.className="celebration-text",a.textContent=t,document.body.appendChild(s),document.body.appendChild(a),setTimeout(()=>s.remove(),700),setTimeout(()=>a.remove(),3100)}async function ut(e,t){e.innerHTML=V(`
    <main class="flex-1 px-6 pb-24 flex items-center justify-center">
      <div class="w-8 h-8 border-2 border-brand-500 border-t-transparent rounded-full animate-spin"></div>
    </main>
  `),W(e);try{const s=await _e(t);if(!s){xt(e,t);return}Ne(s.id),mt(e,s)}catch(s){ft(e,s)}}function mt(e,t){const s=ye(t.id),a=t.resolvedItems.length;let n=-1;e.innerHTML=V(`
  ...
  `}function W(e){e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/programs"))}function xt(e,t){e.innerHTML=V(`
    <main class="flex-1 px-6 pt-12 pb-24">
      <div class="card p-6">
  ...
  `),W(e)}function ft(e,t){e.innerHTML=V(`
    <main class="flex-1 px-6 pt-12 pb-24">
      <div class="card p-6">
  ...
  `),W(e)}function O(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function gt(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/")),e.querySelector('[data-action="new-program"]')?.addEventListener("click",()=>v("/studio/program")),e.querySelector('[data-action="new-exercise"]')?.addEventListener("click",()=>v("/studio/exercise")),e.querySelector('[data-action="ai-builder"]')?.addEventListener("click",()=>v("/studio/ai"))}let U=null;async function bt(){if(U)return U;const{exercises:e}=await A();return U=e.map(t=>({id:t.id,name:t.name,hasDemos:(t.demos||[]).length>0,tokens:I(t.name).concat((t.aliases||[]).flatMap(s=>I(s))).concat(I(t.id.replace(/[-_]/g," "))),exercise:t})),U}function vt(e){U&&U.push({id:e.id,name:e.name,hasDemos:(e.demos||[]).length>0,tokens:I(e.name).concat((e.aliases||[]).flatMap(t=>I(t))).concat(I(e.id.replace(/[-_]/g," "))),exercise:e})}async function K(e,t=10){const s=await bt();if(!e||!e.trim())return s.slice(0,t);const a=I(e);return s.map(n=>({...n,score:yt(n.tokens,a)})).filter(n=>n.score>0).sort((n,r)=>r.score-n.score).slice(0,t)}function ht(e,t={}){e.innerHTML=`
    <div class="space-y-3">
      <div class="relative">
  ...
  `;const s=e.querySelector('[data-input="search"]'),a=e.querySelector('[data-region="results"]'),n=e.querySelector('[data-region="empty"]'),r=e.querySelector('[data-action="create-new"]');let o=null;const i=(l,u)=>{if(!u||!u.trim()){a.classList.add("hidden"),n.classList.add("hidden"),r.classList.add("hidden");return}if(r.classList.remove("hidden"),l.length===0){a.classList.add("hidden"),n.classList.remove("hidden");return}n.classList.add("hidden"),a.classList.remove("hidden"),a.innerHTML=l.map(c=>`
    ...
    `,n()}function a(r,o){const d=(Z.find(u=>u.value===r.type)||Z[0]).fields.includes("startTime"),l=r.type==="youtube"?kt(r.url):null;return`
      <div class="bg-slate-800/40 border border-slate-700/50 rounded-xl p-3 space-y-2.5" data-demo-index="${o}">
        <div class="flex items-center gap-2">
          ...
          <select data-demo-field="type" data-index="${o}" class="flex-1 bg-slate-800/60 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-100 focus:outline-hidden focus:border-brand-500">
            ${Z.map(u=>`<option value="${u.value}"${r.type===u.value?" selected":""}>${u.label}</option>`).join("")}
          </select>
  ...
  `,Pt(e),It(e),At(e),E(e),zt(e),Ht(),t&&Ut(e,t,s)}function Pt(e){e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/studio"))}function It(e){const t=e.querySelector('[data-field="title"]'),s=e.querySelector('[data-field="requirements"]'),a=e.querySelector('[data-region="id-preview"]'),n=e.querySelector('[data-region="export-section"]');t?.addEventListener("input",()=>{m.meta.title=t.value,m.meta.id=jt(t.value),a.textContent=m.meta.id?`id: ${m.meta.id}`:"",n?.classList.toggle("hidden",!m.meta.title.trim()||m.items.length===0)}),s?.addEventListener("input",()=>{m.meta.requirements=s.value})}function At(e){const t=e.querySelector('[data-region="picker"]');ht(t,{onSelect:s=>{m.items.push({type:"single",exerciseId:s.id,exerciseName:s.name,exerciseNote:s.recommendations?.note||"",reps:s.recommendations?.reps||"",sets:s.recommendations?.sets||"",repUnits:s.recommendations?.repUnits||"reps",note:"",tags:[]}),E(e)},onCreateNew:s=>{_t(e,s)}})}function E(e){const t=e.querySelector('[data-region="timeline"]'),s=e.querySelector('[data-region="empty-timeline"]'),a=e.querySelector('[data-region="item-count"]'),n=e.querySelector('[data-region="export-section"]');if(t){if(a.textContent=`${m.items.length} item${m.items.length!==1?"s":""}`,m.items.length===0){t.classList.add("hidden"),s.classList.remove("hidden"),n?.classList.add("hidden");return}if(t.classList.remove("hidden"),s.classList.add("hidden"),n?.classList.toggle("hidden",!m.meta.title.trim()),$t(t,{items:m.items,expandedIndex:b},{onEdit:r=>{b=b===r?-1:r,E(e)},onRemove:r=>{m.items.splice(r,1),b===r?b=-1:b>r&&b--,E(e)},onMove:(r,o)=>{const[i]=m.items.splice(r,1);m.items.splice(o,0,i),b===r?b=o:r<b&&o>=b?b--:r>b&&o<=b&&b++,E(e)},onGroup:(r,o,i)=>{const d=o==="above"?r-1:r+1,l=Math.min(r,d),u=[m.items[l],m.items[l+1]],c={type:"group",kind:i,note:"",tags:[],members:u};m.items.splice(l,2,c),b=-1,E(e)},onJoinGroup:(r,o)=>{const i=m.items[r];m.items[o].members.push(i),m.items.splice(r,1),b=-1,E(e)},onAbsorbIntoGroup:(r,o)=>{const i=m.items[o],d=m.items[r];o<r?d.members.unshift(i):d.members.push(i),m.items.splice(o,1),b=-1,E(e)},onMemberMove:(r,o,i)=>{const d=m.items[r];if(!d||d.type!=="group")return;const[l]=d.members.splice(o,1);d.members.splice(i,0,l),E(e)},onMemberRemove:(r,o)=>{const i=m.items[r];if(!i||i.type!=="group")return;const[d]=i.members.splice(o,1);if(i.members.length<=1){const l=i.members[0]||d;l.type="single",m.items.splice(r,1,l)}b=-1,E(e)},onUngroup:r=>{const o=m.items[r];if(o.type!=="group")return;const i=o.members.map(d=>({...d,type:"single"}));m.items.splice(r,1,...i),b=-1,E(e)}}),b>=0&&b<m.items.length){const r=t.querySelector(`[data-region="edit-form"][data-idx="${b}"]`);r&&Bt(r,m.items[b],b,e)}}}function Bt(e,t,s,a){const n=["reps","secs","min","yd","rep","reps (each side)","secs (each side)"],r=["warmup","stretch"];e.innerHTML=`
...
`),t}async function rs(e,t,s,a,n){const r=ne();if(!r)throw new Error("No API key configured");const i=[{role:"system",content:ss(a)+as(s)},...t.slice(-20),{role:"user",content:e}];let d=await fe(r,i),l=d.choices[0].message,u=0;for(;l.tool_calls&&u<5;){u++;const c=[];for(const p of l.tool_calls){const f=JSON.parse(p.function.arguments),g=await ts(p.function.name,f,s);c.push({role:"tool",tool_call_id:p.id,content:JSON.stringify(g)}),n?.({type:"tool",name:p.function.name,args:f,result:g})}i.push(l),i.push(...c),d=await fe(r,i),l=d.choices[0].message}return l.content||""}async function fe(e,t){const s=await fetch(Kt,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:Zt,messages:t,tools:es,tool_choice:"auto",temperature:.3})});if(!s.ok){const a=await s.text();throw new Error(`OpenAI API error (${s.status}): ${a}`)}return s.json()}function ns(e){const t={id:e.meta.id||"untitled",title:e.meta.title||"Untitled Program"};return e.meta.requirements&&(t.requirements=e.meta.requirements),t.items=e.items,{program:t,newExercises:e.newExercises}}async function os(e){const t=Xt(),s=[];let a=!1;const{exercises:n}=await A(),r=n.map(x=>({id:x.id,name:x.name}));e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/studio"));const o=e.querySelector('[data-region="messages"]'),i=e.querySelector('[data-input="message"]'),d=e.querySelector('[data-action="send"]'),l=e.querySelector('[data-action="export"]'),u=e.querySelector('[data-region="settings-modal"]'),c=e.querySelector('[data-region="export-modal"]');e.querySelector('[data-action="settings"]')?.addEventListener("click",()=>{e.querySelector('[data-input="api-key"]').value=ne(),u.classList.remove("hidden")}),e.querySelector('[data-action="close-settings"]')?.addEventListener("click",()=>u.classList.add("hidden")),e.querySelector('[data-action="save-settings"]')?.addEventListener("click",()=>{const x=e.querySelector('[data-input="api-key"]').value.trim();Qt(x),u.classList.add("hidden")}),u?.addEventListener("click",x=>{x.target===u&&u.classList.add("hidden")}),l?.addEventListener("click",()=>L()),e.querySelector('[data-action="close-export"]')?.addEventListener("click",()=>c.classList.add("hidden")),c?.addEventListener("click",x=>{x.target===c&&c.classList.add("hidden")}),xe()||u.classList.remove("hidden");async function p(){const x=i.value.trim();if(!x||a)return;if(!xe()){u.classList.remove("hidden");return}i.value="",a=!0,d.disabled=!0,f("user",x),s.push({role:"user",content:x});const k=f("assistant","...");k.dataset.loading="true";try{const $=await rs(x,s,t,r,h=>{h.type==="tool"&&g(h)});k.remove(),f("assistant",$),s.push({role:"assistant",content:$}),y()}catch($){k.remove(),f("error",$.message)}finally{a=!1,d.disabled=!1,i.focus()}}d?.addEventListener("click",p),i?.addEventListener("keydown",x=>{x.key==="Enter"&&!x.shiftKey&&(x.preventDefault(),p())});function f(x,k){const $=o.querySelector(".text-center.py-8");$&&x!=="error"&&$.remove();const h=document.createElement("div");h.className=x==="user"?"flex justify-end":"flex justify-start";const M=document.createElement("div");return x==="user"?M.className="bg-brand-500/20 text-slate-100 rounded-2xl rounded-br-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":x==="error"?M.className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed":M.className="bg-slate-800/60 text-slate-200 rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%] text-sm leading-relaxed",M.textContent=k,h.appendChild(M),o.appendChild(h),o.scrollTop=o.scrollHeight,h}function g(x){const k=document.createElement("div");k.className="flex justify-start";const $={search_exercises:`🔍 Searching: "${x.args.query}"`,add_exercise:`✓ Added: ${x.args.exerciseId}`,create_exercise:`✓ Created: ${x.args.name}`,remove_exercise:`✗ Removed item at position ${x.args.index}`,group_exercises:`⚡ Grouped as ${x.args.kind}`,set_metadata:`📝 Updated: ${x.args.title||x.args.requirements||"metadata"}`,update_exercise:`✏️ Updated item at position ${x.args.index}`}[x.name]||`🔧 ${x.name}`;k.innerHTML=`<span class="text-[11px] text-slate-500 italic px-2 py-1">${B($)}</span>`,o.appendChild(k),o.scrollTop=o.scrollHeight}function y(){const x=e.querySelector('[data-region="program-preview"]'),k=e.querySelector('[data-region="program-items"]'),$=e.querySelector('[data-region="item-count"]');if(t.items.length===0){x.classList.add("hidden"),l.classList.add("hidden");return}x.classList.remove("hidden"),l.classList.remove("hidden"),$.textContent=`${t.items.length} item${t.items.length!==1?"s":""}`,k.innerHTML=t.items.map((h,M)=>{if(h.kind)return`<div class="text-xs text-slate-400 pl-2 border-l-2 border-brand-500"><span class="text-brand-300 font-medium">${h.kind}</span>: ${h.exercises.map(je=>je.exerciseId).join(" + ")}</div>`;const T=h.tags?.length?`<span class="text-brand-300">[${h.tags.join(", ")}]</span> `:"";return`<div class="text-xs text-slate-300">${M+1}. ${T}${B(h.exerciseId)} — ${h.reps||"?"} ${h.repUnits||"reps"} × ${h.sets||"?"}</div>`}).join("")}function L(){const{program:x,newExercises:k}=ns(t),$=e.querySelector('[data-region="export-content"]');let h="";if(k.length>0){const T=JSON.stringify(k,null,2);h+=`
        <div class="space-y-2">
          <div class="flex items-center justify-between">
    ...
    `,$.innerHTML=h,$.querySelectorAll("[data-copy]").forEach(T=>{T.addEventListener("click",()=>{navigator.clipboard?.writeText(T.dataset.copy).then(()=>{T.textContent="✓ Copied",setTimeout(()=>{T.textContent="Copy"},2e3)})})}),c.classList.remove("hidden")}}function B(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function is(e){e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-2 flex items-center gap-3 sticky top-0 bg-slate-950/85 backdrop-blur-md z-20 border-b border-slate-900">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/"));const{exercises:t}=await A(),s=e.querySelector('[data-region="count"]');s.textContent=`${t.length} total`;let a=null;const n=l=>{const u=e.querySelector('[data-region="list"]'),c=e.querySelector('[data-region="empty"]');if(l.length===0){u.classList.add("hidden"),c.classList.remove("hidden");return}if(u.classList.remove("hidden"),c.classList.add("hidden"),u.innerHTML=l.map(p=>`
      <li>
    ...
    `).join(""),u.querySelectorAll('[data-action="expand"]').forEach(p=>{p.addEventListener("click",()=>{if(a=a===p.dataset.id?null:p.dataset.id,n(l),a){const f=u.querySelector(`[data-demo-slot="${a}"]`),g=l.find(y=>y.id===a);f&&g&&g.demos.length>0&&H(f,g.demos)}})}),a){const p=u.querySelector(`[data-demo-slot="${a}"]`),f=l.find(g=>g.id===a);p&&f&&f.demos.length>0&&H(p,f.demos)}};function r(l){return`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back());const s=await He(t);if(!s){cs(e,t);return}const{programs:a}=await _(),n=ps(a,t);ds(e,s,n)}function ds(e,t,s){const a=t.demos||[],n=t.recommendations||{},r=t.aliases||[];if(e.innerHTML=`
    <div class="flex-1 flex flex-col">
      <header class="px-6 pt-12 pb-4 flex items-center gap-3">
        <button data-action="back" class="btn-ghost -ml-2 px-3" aria-label="Back">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>window.history.back()),e.querySelectorAll("[data-program-id]").forEach(o=>{o.addEventListener("click",()=>v(`/program/${o.dataset.programId}`))}),a.length>0){const o=e.querySelector('[data-region="demos"]');H(o,a)}}function cs(e,t){e.innerHTML=`
  ...
  `,e.querySelector('[data-action="back"]')?.addEventListener("click",()=>v("/"));const[{programs:t},{plans:s}]=await Promise.all([_(),ke()]),a=t.map(c=>({id:c.id,title:c.title,requirements:c.requirements||"",itemCount:c.items?.length||c.exercises?.length||0,tokens:G(c.title).concat(G(c.requirements||"")).concat(G(c.id.replace(/[-_]/g," "))),program:c})),n=new Map;for(const c of s)for(const p of c.subPlans||[])for(const f of p.programs||[])n.set(f,`${c.name} · ${p.name}`);const r=e.querySelector('[data-region="results"]'),o=e.querySelector('[data-region="empty"]'),i=e.querySelector('[data-region="initial"]');function d(c){if(c===null){r.classList.add("hidden"),o.classList.add("hidden"),i.classList.remove("hidden");return}if(i.classList.add("hidden"),c.length===0){r.classList.add("hidden"),o.classList.remove("hidden");return}o.classList.add("hidden"),r.classList.remove("hidden"),r.innerHTML=c.map(p=>ms(p,n.get(p.id))).join(""),r.querySelectorAll("[data-program-id]").forEach(p=>{p.addEventListener("click",()=>v(`/program/${p.dataset.programId}`))})}const l=e.querySelector('[data-input="search"]');let u=null;l?.addEventListener("input",()=>{clearTimeout(u),u=setTimeout(()=>{const c=l.value.trim();if(!c){d(null);return}const p=G(c),f=a.map(g=>({...g,score:xs(g.tokens,p)})).filter(g=>g.score>0).sort((g,y)=>y.score-g.score);d(f)},150)}),d(null)}function ms(e,t){return`
    <button
      data-program-id="${e.id}"
      class="w-full card p-4 text-left active:scale-[0.98] transition-transform"
    >
  ...
  `}function G(e){return e?e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>0):[]}function xs(e,t){let s=0;for(const a of t){let n=0;for(const r of e)r===a?n=Math.max(n,10):r.startsWith(a)?n=Math.max(n,7):r.includes(a)&&(n=Math.max(n,4));if(n===0)return 0;s+=n}return s}function te(e){return e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const C=document.getElementById("app");j("/",()=>Re(C));j("/programs",()=>Je(C));j("/program/:id",({id:e})=>ut(C,e));j("/exercises",()=>is(C));j("/exercise/:id",({id:e})=>ls(C,e));j("/search",()=>us(C));const fs=["localhost","127.0.0.1"].includes(window.location.hostname);fs&&(j("/studio",()=>gt(C)),j("/studio/program",()=>Tt(C)),j("/studio/exercise",()=>Ft(C)),j("/studio/ai",()=>os(C)));Te(e=>{C.innerHTML=`
    <div class="flex-1 flex flex-col items-center justify-center min-h-screen px-6 text-center">
      <p class="text-6xl mb-4">🤔</p>
